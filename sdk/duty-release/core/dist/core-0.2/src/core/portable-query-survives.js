...
const eventsMatching = (state, reference, predicate) => {
    const result = [];
    for (let position = 0; position < state.events.length; position += 1) {
        if (predicate(state.events[position]))
            result.push(reference(position));
    }
    return result;
};
/** The relevance relation includes historical assignments, even after closure. */
export const portableObligationLinksTarget = (state, obligationId, targetAgentId) => {
    ...
    if (state.intentDeclarations.get(record.sourceIntentId)?.data.actorId === targetAgentId ||
        ...
        record.performanceAssigneeId === targetAgentId)
        return true;
    for (const event of state.events) {
        if (event.type === "OBLIGATION_PERFORMANCE_ASSIGNED" && event.data.obligationId === obligationId &&
            event.data.toAgentId === targetAgentId)
            return true;
    }
    return false;
};
const parentPath = (state, record) => {
    const result = [];
    let cursor = record;
...
const dependencies = (state, path) => {
    ...
    for (let index = 0; index < queue.length; index += 1) {
        ...
        if (seen.has(record.grant.authorityId))
            continue;
        ...
        for (const identifier of record.grant.constraints.requiredIntersectionIds) {
            ...
            if (required?.grant.kind === "PERMISSION")
                for (const dependency of parentPath(state, required)) {
                    if (enqueued.has(dependency.grant.authorityId))
                        continue;
                    enqueued.add(dependency.grant.authorityId);
...
const leastEvidence = (references) => {
    let least = references[0];
    for (const reference of references)
        if (compareQueryEvidence(reference, least) < 0)
            least = reference;
    return least;
};
const skeletonFor = (state, targetAgentId) => ({
...
const walkPortableSurvives = (state, targetAgentId, evaluationTime, writer) => {
    ...
    const eventReference = (position) => cache.event(state, position);
    const receiptReference = cache.receipt;
    const matching = (predicate) => eventsMatching(state, eventReference, predicate);
    if (agent === undefined)
        return;
    writer.evidence("historicalIdentity", matching(event => (event.type === "PRINCIPAL_CREATED" && event.data.principalId === agent.principalId) ||
    ...
    for (const tenure of state.tenures.values()) {
        if (tenure.agentId !== targetAgentId)
            continue;
        const evidence = matching(event => (event.type === "ROLE_CREATED" && event.data.roleId === tenure.roleId) ||
        ...
        writer.row(tenure.closed ? "transferredRoleTenures" : "currentRoleTenures", tenure.id, { roleId: tenure.roleId, agentId: targetAgentId, roleTenureId: tenure.id,
            tenureNumber: tenure.tenureNumber, status: tenure.closed ? "CLOSED" : "CURRENT" }, evidence);
    }
    const includedIntentIds = new Set();
    for (const [intentId, declaration] of state.intentDeclarations) {
        if (declaration.data.actorId !== targetAgentId)
            continue;
    ...
    for (const [dutyId, duty] of state.attemptDuties) {
        ...
        writer.row("attemptDuties", dutyId, {
            ...
            ...(currentView ? [eventReference(currentView.policy.eventPosition), ...state.events.flatMap((event, position) => (event.type === "ATTEMPT_DUTY_DISPOSITION_RECORDED" || event.type === "ATTEMPT_DUTY_CONTEST_RECORDED") && event.data.dutyId === dutyId ? [eventReference(position)] : [])] : []),
    ...
    for (const [obligationId, obligation] of state.obligations) {
        if (obligation.status === "DISCHARGED" || obligation.status === "IMPOSSIBLE_OR_ESCALATED" ||
            !portableObligationLinksTarget(state, obligationId, targetAgentId))
            continue;
        const record = obligation.record;
        ...
        const targetLinks = matching(event => (event.type === "TRANSACTION_INTENT_DECLARED" && event.data.intentId === record.sourceIntentId && event.data.actorId === targetAgentId) ||
        ...
        writer.row("obligations", obligationId, { obligationId, durableRoleId: record.durableRoleId,
            performanceAssigneeId: obligation.performanceAssigneeId, status: obligation.status, deadline: record.deadline }, [...causal, ...assignments, ...statuses, ...targetLinks]);
        let successionRuleId = record.successionRuleId;
        for (const event of state.events) {
            if (event.type === "OBLIGATION_PERFORMANCE_ASSIGNED" && event.data.obligationId === obligationId)
                successionRuleId = event.data.successionRuleId;
        }
        writer.row("currentPerformanceAssignments", obligationId, { obligationId, assigneeId: obligation.performanceAssigneeId, successionRuleId }, [...causal, ...assignments]);
        includedReceiptHashes.add(record.causalReceiptContentHash);
    }
    for (const receipt of state.receiptCommitments.values()) {
        if (includedIntentIds.has(receipt.intentId) || includedReceiptHashes.has(receipt.receiptContentHash))
            writer.evidence("receiptCommitments", [receiptReference(receipt)]);
    }
    for (const [authorityId, record] of state.authorities) {
        ...
        if (!path.some(item => item.grant.kind === "PERMISSION" &&
            (item.grant.grantorId === targetAgentId || item.grant.granteeId === targetAgentId)))
            continue;
        ...
        for (const item of graph) {
            if (item.grant.kind !== "PERMISSION")
                continue;
            for (const id of [item.grant.grantorId, item.grant.granteeId]) {
                if (state.agents.get(id)?.terminated === true)
                    terminatedIds.add(id);
            }
        }
        const terminations = matching(event => event.type === "AGENT_TERMINATED" && terminatedIds.has(event.data.agentId));
        if (terminations.length !== 0)
            writer.row("invalidatedAuthorityDependencies", authorityId, { authorityId, state: "AGENT_TERMINATED" }, [...pathEvidence, leastEvidence(terminations)]);
    }
};
const sortSurvives = (answer) => {
    ...
    answer.transferredRoleTenures.sort(compareTenures);
    answer.unresolvedIntents.sort((a, b) => compareProtocolStrings(a.intentId, b.intentId));
    answer.adapterOutcomes.sort((a, b) => compareProtocolStrings(a.intentId, b.intentId));
    answer.obligations.sort((a, b) => compareProtocolStrings(a.obligationId, b.obligationId));
    answer.currentPerformanceAssignments.sort((a, b) => compareProtocolStrings(a.obligationId, b.obligationId));
    answer.invalidatedAuthorityDependencies.sort((a, b) => compareProtocolStrings(a.authorityId, b.authorityId));
};
export const derivePortableSurvives = (state, targetAgentId, evaluationTime) => {
...
export const projectPortableSurvives = (state, targetAgentId, evaluationTime) => requirePortableQueryProjection(derivePortableSurvives(state, targetAgentId, evaluationTime));
export const portableSurvivesAnswersDiffer = (left, right, targetAgentId, evaluationTime) => {
    if (canonicalEncode(skeletonFor(left, targetAgentId)) !== canonicalEncode(skeletonFor(right, targetAgentId)))
        return true;
