...
export function dutyReservationOf(events) {
    ...
    for (const event of events) {
        ...
        if (event.type === "ATTEMPT_DUTY_POLICY_ACTIVATED") {
            ...
            if (!duties.has(d.descriptor.dutyId))
                duties.set(d.descriptor.dutyId, { dispositions: 0, contests: 0 });
        }
            ...
            if (event.type === "ATTEMPT_DUTY_DISPOSITION_RECORDED")
                duty.dispositions++;
            else
                duty.contests++;
            if (duty.dispositions > 4 || duty.contests > 2)
                withinLimits = false;
        }
    }
    // An invalid over-quota candidate must never manufacture negative reserves.
    const dutyReserved = [...duties.values()].reduce((sum, duty) => sum + Math.max(0, 4 - duty.dispositions) + Math.max(0, 2 - duty.contests), 0);
...
export function capacityOf(events) {
    ...
    for (const event of events) {
        if (!bounded(event))
            shape = false;
        const d = event.data;
        if (dutyExtension.has(event.type))
            extended = true;
...
export function assertCapacityTransition(events, next) {
    ...
    if (next.some(event => dutyExtension.has(event.type)))
        error("CAPACITY_INCOMPATIBLE");
