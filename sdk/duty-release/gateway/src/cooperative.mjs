...
import {inspectContinuationDutyPolicy} from '@ramex-labs/continuity/duties';
import {openLocalAttemptRecorder} from '@ramex-labs/continuity';
import {openConfiguredEventStore,observeConfiguredStore,exportContinuationEvents,stateOf,canonicalEncode,openLocalExecution,derivePortableAdapterIdentity,portableAdmissionControlIsCurrent,evaluatePortableReceiptPolicy,
...
export async function createCooperativeGateway(options){
 ...
 const store=openConfiguredEventStore(local),jobs=new Map(),business=new Set();
 ...
 function policy(job,action,resource=job.toolDefinition.resource,projection={}){
  ...
  const decision=observeConfiguredStore(store,{at}).authorize({actor:session.agentId,action,resource,...projection});
 ...
 const investigation=job=>{
  const snapshot=store.directoryStore?.snapshot(),state=snapshot?stateOf(exportContinuationEvents(snapshot.history)):read();
  check(same(state.genesis.domain,local.domain),'PROFILE_MISMATCH');
  const duty=[...state.attemptDuties.entries()].find(([,value])=>value.record.sourceIntentId===job.operationId);
  if(!duty||!state.attemptDutyPolicies.has(duty[0]))return {};
  check(store.directoryStore,'UNSUPPORTED_DUTY_HISTORY_PROFILE');
  return {investigation:inspectContinuationDutyPolicy(snapshot.history,duty[0]).policy};
 };
 const summary=(job,reply,status)=>({...investigation(job),status,operationId:job.operationId,serviceState:reply?.result.state??null,
 ...
 return Object.freeze({
  ...
  run(name,input,{assertCurrent=()=>{},onProgress=()=>{}}={}){return locked(async()=>{
   ...
   for(const method of ['checkpoint','checkpointHistory','prepare','commit'])guardedClient[method]=async value=>{assertCurrent();dispatchPolicy(job);onProgress(method==='checkpointHistory'?'checkpoint':method);assertCurrent();dispatchPolicy(job);if(method==='prepare'||method==='commit')dispatchStarted=true;return client[method](value);};
