...
try{
 ...
 for(let i=0;i<args.length;i+=2){const key={'--history':'historyFile','--history-binding':'historyBinding','--storage':'storage','--directory':'directory'}[args[i]];if(!key||options[key])throw Error();options[key]=args[i+1];}
 if(options.historyBinding)options.historyProfile='continuity-segmented-local/1';
 const allowed={inspect:options.historyBinding?['historyBinding','historyProfile','storage']:['historyFile','storage'],snapshot:['historyFile','storage','directory'],verify:['directory'],'recover-dead-host':['storage']}[command];
