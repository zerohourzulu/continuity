...
import {openConfiguredEventStore,observeConfiguredStore,stateOf,openLocalExecution,derivePortableAdapterIdentity,
...
export async function createGateway(options){
 ...
 const inspectPolicy=(job,action='inspect-operation',termsCommitment)=>{
  ...
  const decision=observeConfiguredStore(store,{at}).authorize({actor:session.agentId,action,resource:job.resource,...(termsCommitment?{termsCommitment}:{})});
