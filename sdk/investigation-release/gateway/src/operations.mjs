...
import {PortableFileEventStore,openConfiguredEventStore,ConfiguredDirectoryEventStore,exportContinuationEvents,stateOf,capacityOf,LOCAL_CAPACITY_PROFILE,CONTINUATION_PROFILE} from '@ramex-labs/continuity/adapter';
...
export function inspectCase(location){
 ...
 const snapshot=store instanceof ConfiguredDirectoryEventStore?store.directoryStore.snapshot():null;
 const state=stateOf(snapshot?exportContinuationEvents(snapshot.history):store.readAll()),names=recordNames(directory);
 let recordBytes=0;
 ...
 if(store instanceof ConfiguredDirectoryEventStore){
  ...
  const views=[...state.attemptDutyPolicies.keys()].map(id=>inspectContinuationDutyPolicy(snapshot.history,id).policy);
  const dutySummary=views.length?{dutySummary:{version:'continuity-duty-summary/1',activated:views.length,outstanding:views.filter(view=>view.outstanding).length,states:Object.fromEntries(['OPEN','COMPLETED_UNDER_POLICY','ESCALATED','NEEDS_REVIEW','CONTESTED'].map(status=>[status,views.filter(view=>view.dutyDisposition===status).length])),externalOutcome:'NOT_PROVEN'}}:{};
  return {...dutySummary,profile:CONTINUATION_PROFILE.version,eventCount:count,limits:{coreEvents:1024,observationEvents:1024,recordEntries:4096},remainingCoreEvents:1024-count,
