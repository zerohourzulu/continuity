...
import { CONTINUATION_PROFILE as P, CONTINUATION_HISTORY_VERSION, captureContinuationHistory, exportContinuationEvents, appendContinuationEvent, continuationPrefix } from "../history.js";
import { historyDataFields, captureHistoryAncillary } from "../../../core-0.2/src/history/index.js";
import { historyCapacity } from "./capacity.js";
import { LIMITS, fail, exact, integer, digest, uuid, json, parse, encodeEvent, decodeEvent } from "./codec.js";
import { sha, directory, read, writeNew, writeImmutable, syncDirectory, rename, withLock } from "./files.js";
export const STORE_FORMAT = 'continuity-directory-history/1';
const same = (a, b) => core.canonicalEncode(a) === core.canonicalEncode(b);
export function bindingContents(path, m) {
    if (!m.migration)
...
export function markerContents(path, m) {
    if (!m.migration)
        fail('MIGRATION_REQUIRED');
...
function decodeManifest(bytes) {
    ...
    if (position !== m.head.position + 1 || canonical !== m.canonicalBytes || nodes !== m.nodes || encoded !== m.encodedBytes)
        fail('MANIFEST_INVALID');
    return core.immutableProtocolValue(m);
}
export function readManifest(path) { directory(path); return decodeManifest(read(join(path, 'manifest.json'), LIMITS.manifestBytes)); }
export function verifyMigrationBinding(path, m) {
    if (!m.migration)
        return;
...
export function readSnapshot(path, allowInactive = false) {
    path = directory(path);
...
function countNodes(v) { return 1 + (Array.isArray(v) ? v.reduce((n, x) => n + countNodes(x), 0) : v && typeof v === 'object' ? Object.values(v).reduce((n, x) => n + 1 + countNodes(x), 0) : 0); }
function segment(events, start) {
...
export function inventory(path, m) {
...
export function publishManifest(path, m, h = {}) {
    const bytes = json(m);
...
export function initializeStore(path, history, options = {}, h = {}) {
    ...
    syncDirectory(path);
    const m = { format: STORE_FORMAT, profile: P.version, instance: options.instance ?? randomUUID(), generation: 0, phase: options.phase ?? 'ACTIVE',
...
export class DirectoryHistoryStore {
    ...
    hooks;
    #uncertain = false;
    constructor(path, hooks = {}) { this.hooks = hooks; this.path = directory(path); this.snapshot(); }
    static create(path, history, options = {}) {
            ...
            return withLock(this.path, () => {
                ...
                const check = () => {
                    if (!active)
                        fail('WRITER_EXPIRED');
                    if (operating)
                        fail('WRITER_REENTRANT');
                    if (this.#uncertain)
                        fail('WRITE_UNCERTAIN');
                };
                const writer = Object.freeze({ snapshot: () => { check(); return this.snapshot(); }, append: (event, expected) => {
                        check();
                        operating = true;
                        try {
                            ...
                            if (orphans.length + 2 > LIMITS.orphanFiles || orphans.reduce((n, x) => n + x.bytes, 0) + c.bytes.length + LIMITS.manifestBytes > LIMITS.orphanBytes)
                                fail('ORPHAN_LIMIT');
                            const manifest = { ...current.manifest, generation: current.revision.generation + 1, head: next.head, canonicalBytes: next.metrics.canonicalBytes, nodes: next.metrics.nodes, encodedBytes: capacity.usage.encodedBytes, segments: [...current.manifest.segments, c.descriptor] };
    ...
    append(event, expected) { return this.withWriter(w => w.append(event, expected)); }
    ...
    cleanupOrphans(expected, selected) {
