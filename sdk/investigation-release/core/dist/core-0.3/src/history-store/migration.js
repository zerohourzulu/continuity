...
import { captureContinuationHistory, CONTINUATION_HISTORY_VERSION } from "../history.js";
import { historyCapacity } from "./capacity.js";
import { LIMITS, fail, exact, digest, uuid, integer, json, parse } from "./codec.js";
import { sha, directory, read, fileInfo, writeImmutable, syncDirectory, rename, withLock } from "./files.js";
import { initializeStore, readSnapshot, readManifest, publishManifest, verifyMigrationBinding, bindingContents, markerContents } from "./store.js";
const same = (a, b) => core.canonicalEncode(a) === core.canonicalEncode(b);
const full = (v) => {
    if (typeof v !== 'string' || !v || v.length > 4096)
        fail('PATH_INVALID');
    const p = resolve(v);
    return join(directory(dirname(p)), basename(p));
};
function sourceHistory(source, expected) {
...
export function readMigrationPlan(path) {
    ...
    return core.immutableProtocolValue(p);
}
function metadata(p) { return { id: p.id, source: p.source, sourceDigest: p.sourceDigest, sourceHead: p.sourceHead, configurationFiles: p.configurationFiles, artifacts: p.artifacts.map(({ source: _source, ...rest }) => rest) }; }
function compatibleSource(p) {
...
export function prepareMigration(input) {
    ...
    return withLock(dirname(source), () => {
        ...
        const plan = { version: 'continuity-history-migration/1', phase: 'PLANNED', id: randomUUID(), instance: randomUUID(), source, target, sourceDigest: sha(bytes), sourceDevice: info.dev, sourceInode: info.ino, sourceHead: history.head, configurationFiles, artifacts };
...
export function stageMigration(planFile, options, hooks = {}) {
...
export function activateMigration(planFile, options, hooks = {}) {
    if (options.quiesced !== true)
        fail('QUIESCENCE_REQUIRED');
...
export function inspectMigration(planFile) {
    ...
    return Object.freeze({ id: p.id, source: p.source, target: p.target, sourcePhase, targetPhase, dispatchReady: targetPhase === 'ACTIVE' && sourcePhase === 'MARKED' ? (() => {
            try {
                checkTarget(p);
