...
const dutyText = (v, value, path, max) => typeof value === "string" && boundedWellFormedUtf8Status(value, max) === "VALID" || v.fail(path, "must be bounded UTF-8 text");
const dutyDigest = (v, value, path) => {
    const r = v.record(value, path, ["algorithm", "value"]);
    return r !== undefined && r.algorithm === "sha256" && contentHash(v, r.value, `${path}.value`);
};
const dutyFinding = (v, value, path) => {
    const f = v.record(value, path, ["challenge", "runtimeSignature"]);
    if (!f || !signature65(v, f.runtimeSignature, `${path}.runtimeSignature`))
        return false;
    ...
    if (!c || c.version !== "continuity-duty-finding/1" || c.rulesHash !== DUTY_POLICY_RULES_HASH ||
        ...
        !u53(v, c.evaluationTime, `${path}.evaluationTime`) || !u53(v, c.controlEpoch, `${path}.controlEpoch`))
        return false;
    for (const key of ["descriptorHash", "evidenceIndexHash", "authorityProofHash"])
        if (!contentHash(v, c[key], `${path}.${key}`))
            return false;
    for (const key of ["activationEventId", "dutyId", "sourceAdmissionEventId", "transitionEventId", "latestAssignmentEventId", "attesterId", "runtimeSessionId", "credentialKeyId", "roleId", "roleTenureId", "attestationAuthorityId", "dispositionAuthorityId"])
        if (!identifier(v, c[key], `${path}.${key}`))
            return false;
    if (c.previousDispositionEventId !== null && !identifier(v, c.previousDispositionEventId, `${path}.previousDispositionEventId`))
        return false;
    if (c.purpose === "DISPOSITION" && c.transitionEventType === "ATTEMPT_DUTY_DISPOSITION_RECORDED") {
        ...
        if (!o || !literal(v, o.disposition, `${path}.disposition`, ["COMPLETED_UNDER_POLICY", "ESCALATED"]) || !dutyDigest(v, o.reportDigest, `${path}.reportDigest`) ||
            (o.nextStep !== null && !dutyText(v, o.nextStep, `${path}.nextStep`, 512)))
            return false;
        const keys = ["SOURCE_REVIEWED", "HISTORY_REVIEWED", "FINDING_RECORDED", "CONTROL_REVIEWED"];
        const checklist = v.record(o.checklist, `${path}.checklist`, keys);
        if (!checklist)
            return false;
        for (const key of keys) {
            const r = v.record(checklist[key], `${path}.${key}`, ["state", "reason"]);
            if (!r || !literal(v, r.state, `${path}.state`, ["SATISFIED", "UNAVAILABLE", "UNRESOLVED"]) || !dutyText(v, r.reason, `${path}.reason`, 256))
                return false;
        }
        return true;
    }
...
for (const type of ["ATTEMPT_DUTY_DISPOSITION_RECORDED", "ATTEMPT_DUTY_CONTEST_RECORDED"]) {
    mapSet(CORE_EVENT_NAMED_LIMITS, type, limitRecord(limitField("actorId", LIMIT_IDENTIFIER), limitField("dutyId", LIMIT_IDENTIFIER), limitField("dispositionAuthorityId", LIMIT_IDENTIFIER), limitField("administrativeAuthorization", LIMIT_ADMINISTRATIVE_AUTHORIZATION), limitField("finding", limitRecord(limitField("challenge", limitRecord(limitField("domain", LIMIT_DOMAIN), ...["activationEventId", "dutyId", "sourceAdmissionEventId", "transitionEventId", "latestAssignmentEventId", "previousDispositionEventId", "attesterId", "runtimeSessionId", "credentialKeyId", "roleId", "roleTenureId", "attestationAuthorityId", "dispositionAuthorityId"].map(key => limitField(key, LIMIT_IDENTIFIER))))))));
}
