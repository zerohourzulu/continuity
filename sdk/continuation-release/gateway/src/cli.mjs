...
try{
 ...
 const config=exact(readPrivate(process.argv[3]),['domain','owner','controller','session','keyFile','storage','upstreams','operations'],['timeoutMs','historyFile','historyBinding','historyProfile']);
 check(config.historyProfile!==undefined||config.historyBinding===undefined,'UNSUPPORTED_HISTORY_PROFILE');
 ...
 const local={...(config.historyProfile===undefined?{historyFile:config.historyFile}:{historyProfile:config.historyProfile,historyBinding:config.historyBinding,...(config.historyFile!==undefined?{historyFile:config.historyFile}:{})}),domain:config.domain,owner:config.owner,controller:config.controller,session:config.session,
