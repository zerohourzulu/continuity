...
import { compareQueryEvidence, queryEventReference } from "./portable-query-codec.js";
import { portableObligationLinksTarget, portableSurvivesAnswersDiffer } from "./portable-query-survives.js";
const proofOf = (event) => event.data.authorizationProof;
const wrapperOf = (event) => event.data.administrativeAuthorization;
const pairFor = (left, x, right, y) => {
    const a = queryEventReference(left, x), b = queryEventReference(right, y);
    return Object.freeze(compareQueryEvidence(a, b) <= 0 ? [a, b] : [b, a]);
};
const leastPair = (previous, candidate) => previous === undefined || compareQueryEvidence(candidate[0], previous[0]) < 0 ||
    (compareQueryEvidence(candidate[0], previous[0]) === 0 && compareQueryEvidence(candidate[1], previous[1]) < 0)
    ? candidate : previous;
const suffixAdmissions = (state, prefixLength) => {
    const result = new Map();
    for (const admission of state.intentAdmissions.values()) {
        if (admission.admissionEventPosition < prefixLength)
            continue;
...
const suffixCreations = (state, prefixLength) => {
    const result = [];
    for (const obligation of state.obligations.values()) {
        if (obligation.creationEventPosition < prefixLength)
            continue;
...
const obligationLinksTargetAtPrefix = (state, obligationId, target, prefixLength) => {
    const obligation = state.obligations.get(obligationId);
    if (obligation === undefined || obligation.creationEventPosition >= prefixLength)
        return false;
    const record = obligation.record;
    if (state.intentDeclarations.get(record.sourceIntentId)?.data.actorId === target ||
        state.tenures.get(record.creationRoleTenureId)?.agentId === target || record.performanceAssigneeId === target)
        return true;
    for (let position = obligation.creationEventPosition + 1; position < prefixLength; position += 1) {
        ...
        if (event.type === "OBLIGATION_PERFORMANCE_ASSIGNED" && event.data.obligationId === obligationId &&
            event.data.toAgentId === target)
            return true;
    }
    return false;
};
/** Recheck the exact causal identity chain, without treating a suffix-created obligation as present at C. */
const obligationLinksIntentAtPrefix = (state, obligationId, intentId, prefixLength) => {
    const obligation = state.obligations.get(obligationId);
    if (obligation === undefined || obligation.creationEventPosition >= prefixLength || obligation.record.sourceIntentId !== intentId)
        return false;
 ...
 */
export const findPortableQueryConflict = (evaluationState, observedState, query) => {
    ...
    if (a.protocol !== b.protocol || a.version !== b.version || a.deploymentId !== b.deploymentId || a.chainId !== b.chainId ||
        ...
        left.genesis.canonicalLineageId !== right.genesis.canonicalLineageId)
        return undefined;
    let prefixLength = 0;
    while (prefixLength < left.events.length && prefixLength < right.events.length &&
        canonicalEncode(left.events[prefixLength]) === canonicalEncode(right.events[prefixLength]))
        prefixLength += 1;
    if (prefixLength === 0 || prefixLength === left.events.length || prefixLength === right.events.length)
        return undefined;
    if (query.kind !== "SURVIVES" && query.binding?.intentId === undefined)
        return undefined;
    if (query.kind === "SURVIVES") {
        if (!Number.isSafeInteger(query.evaluationTime) || query.evaluationTime < left.head.canonicalTime ||
            query.evaluationTime < right.head.canonicalTime)
            throw new TypeError("Conflict projection requires causal time at both heads.");
        if (!portableSurvivesAnswersDiffer(left, right, query.targetAgentId, query.evaluationTime))
            return undefined;
    }
    const admissionRelevant = (x, y) => query.kind === "SURVIVES"
    ...
    const creationRelevant = (x, y) => query.kind === "SURVIVES"
    ...
    const laterRelevant = (x, y) => {
        const obligationId = x.data.obligationId;
    ...
    const rightNonces = new Map();
    for (const y of rightAdmissions.values())
        rightNonces.set(canonicalEncode([y.intent.actorId, y.intent.nonce]), y);
    for (const x of leftAdmissions.values()) {
    ...
    const boundToCommon = (proof) => proof.historyHead.position === prefixLength - 1 && proof.historyHead.hash === commonHash &&
    ...
    if (firstLeft.type === "TRANSACTION_INTENT_ADMITTED" && firstRight.type === "TRANSACTION_INTENT_ADMITTED") {
        const x = leftAdmissions.get(firstLeft.data.intentId), y = rightAdmissions.get(firstRight.data.intentId);
        if (x.intent.intentId !== y.intent.intentId &&
            ...
            for (const id of shared) {
                ...
                if (authority.grant.kind !== "PERMISSION" || authority.grantEventPosition >= prefixLength || prior === undefined)
                    continue;
    ...
    const rightCreations = suffixCreations(right, prefixLength), rightSources = new Map(), rightObligations = new Map();
