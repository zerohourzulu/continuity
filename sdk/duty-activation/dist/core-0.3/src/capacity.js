...
export function capacityOf(events) {
    ...
    for (const event of events) {
        ...
        if (dutyExtension.has(event.type))
            extended = true;
        // D02 has activation only. Replay validates selection/uniqueness; this is a
        // census, not authority. Never use caller-provided limits to reduce room.
        if (event.type === "ATTEMPT_DUTY_POLICY_ACTIVATED")
            activatedDuties.add(d.descriptor.dutyId);
    ...
    const dutyReserved = activatedDuties.size * 6;
    ...
    return Object.freeze({ profile: LOCAL_CAPACITY_PROFILE.version, eventCount: events.length,
        ...
        ...(activatedDuties.size ? { activatedDuties: activatedDuties.size, dutyReserved } : {}),
