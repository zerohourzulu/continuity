import { constants, openSync, closeSync, readSync, writeFileSync, fsyncSync, lstatSync, fstatSync, opendirSync, mkdirSync, } from 'node:fs';
import { resolve, parse, join, sep, dirname } from 'node:path';
import { createHash } from 'node:crypto';
import { derivePortableAdapterIdentity } from "../../../core-0.2/src/core/portable-integration.js";
import { hashCanonical, immutableProtocolInput } from "../../../core-0.2/src/core/canonical.js";
import { approvedPortableAdapterProfile, LOCAL_EVIDENCE_PACKET_ADAPTER_ID, createPortableAdapterAcknowledgment, portableAdapterAcknowledgmentEvidence, validatePortableAdapterAcknowledgment } from "../../../core-0.2/src/core/portable-adapter-engine.js";
const VERSION = '0.2';
...
function lexicalDirectory(path) {
    if (typeof path !== 'string' || !path || path.includes('\0') || path.split(sep).includes('..'))
...
function directory(path) {
    ...
    for (const component of absolute.slice(root.length).split(sep).filter(Boolean)) {
        ...
        if (stat.isSymbolicLink() || !stat.isDirectory())
            fail('Directory path must not contain a symlink or non-directory');
    }
    return absolute;
}
    ...
    try {
        for (let entry; (entry = handle.readSync()) !== null;) {
            ...
            if (result.length > maximum)
                fail('Too many directory entries');
        }
    }
    finally {
        handle.closeSync();
...
function readRegular(path, maximum) {
    ...
    if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1 || before.size > maximum)
        fail('Expected bounded regular non-linked file');
    ...
    try {
        ...
        if (length > maximum || length !== opened.size || after.size !== opened.size || after.mtimeMs !== opened.mtimeMs ||
            after.ctimeMs !== opened.ctimeMs || after.nlink !== 1 || named.isSymbolicLink() || named.dev !== opened.dev || named.ino !== opened.ino)
            fail('File changed during bounded read');
...
function selectionValue(value) {
    ...
    if (!Array.isArray(stable) || stable.length < 1 || stable.length > 8)
        fail('Select between one and eight files');
    ...
    for (const item of stable) {
        if (!exactKeys(item, ['name', 'bytes', 'sha256']) || typeof item.name !== 'string' || !NAME.test(item.name) ||
            ...
            typeof item.sha256 !== 'string' || !DIGEST.test(item.sha256) || (previous !== undefined && previous >= item.name))
            fail('Invalid ordered file selection');
        previous = item.name;
        total += item.bytes;
    }
    if (total > TOTAL_BYTES)
        fail('Selected files exceed aggregate byte limit');
...
export function selectInput(inputDirectory) {
    ...
    const selected = names.map(name => {
        if (!NAME.test(name))
            fail('Invalid selected basename');
...
function readRecord(path) {
    ...
    if (!bytes.equals(jsonBytes(value)))
        fail('Non-exact retained record encoding');
    return { value, bytes };
}
export function createPacketExecutor({ inputDirectory, outputDirectory, selection, termsCommitment, resource, domain, intentId: configuredIntentId }) {
    ...
    if (!expectedDomain || expectedDomain.version !== VERSION || typeof configuredIntentId !== 'string' || !configuredIntentId)
        fail('Missing fixed executor domain/intent');
    ...
    if (input === output || input.startsWith(output + sep) || output.startsWith(input + sep))
        fail('Input and output directories must be disjoint');
    const selected = selectionValue(selection);
    if (typeof termsCommitment !== 'string' || !HASH.test(termsCommitment) || typeof resource !== 'string' ||
        resource.length < 1 || resource.length > 256 || Buffer.byteLength(resource) > 256 || /[\u0000-\u001f\u007f]/u.test(resource))
        fail('Invalid executor terms or resource');
    ...
    function retainedAttempt() {
        ...
        if (!exactKeys(value, ['schemaVersion', 'idempotencyKey', 'submissionFingerprint', 'intentId', 'selection', 'termsCommitment', 'resource', 'adapterIdentity']) ||
            ...
            value.intentId !== configuredIntentId || value.adapterIdentity.intentId !== configuredIntentId || !equal(value.adapterIdentity.domain, expectedDomain) || !equal(value.adapterIdentity.adapterProfile, adapterProfile))
            fail('Attempt identity/configuration mismatch');
        return value;
    }
    function inspect() {
        try {
            if (!equal(entries(output, 3), ['ack.json', 'attempt.json', 'packet']))
                fail('Missing or unexpected output entries');
            const attempt = retainedAttempt();
            if (!equal(entries(packet, 2), ['files', 'manifest.json']) || !equal(entries(files, 8), selected.map(item => item.name)))
                fail('Packet file membership mismatch');
            const { value: manifest, bytes: manifestBytes } = readRecord(join(packet, 'manifest.json'));
            if (!equal(manifest, record(MANIFEST, attempt.adapterIdentity, attempt.intentId)))
                fail('Manifest does not bind the retained attempt');
            for (const item of selected) {
                const bytes = readRegular(join(files, item.name), FILE_BYTES);
                if (bytes.length !== item.bytes || sha256(bytes) !== item.sha256)
                    fail('Packet file hash mismatch');
            }
            ...
            if (!validatePortableAdapterAcknowledgment(typed, attempt.adapterIdentity) || !equal(ack, { schemaVersion: ACK, idempotencyKey: attempt.idempotencyKey,
                submissionFingerprint: attempt.submissionFingerprint, manifestSha256, acknowledgment: typed, evidence: evidence(attempt.adapterIdentity, typed) }))
                fail('Acknowledgment mismatch');
            return freeze({ status: 'PACKET_VERIFIED', manifestSha256, manifest, files: selected, acknowledgment: typed });
        }
        catch (error) {
            return unknown(error instanceof Error ? error.message : 'Retained packet unavailable');
        }
    }
    function captureSubmission(submission) {
        ...
        if (!equal(identity.adapterProfile, adapterProfile) || !equal(identity.domain, expectedDomain) || identity.intentId !== configuredIntentId)
            fail('Submission differs from fixed executor profile/domain/intent');
        const request = stable.admissionEvent.data.authorizationProof.request;
        if (request.action !== 'collect-evidence-packet' || request.resource !== resource || request.termsCommitment !== termsCommitment)
            fail('Admitted request does not match the configured executor');
    ...
    function retained(identity, intentId) {
        ...
        try {
            ...
            return unknown('Original attempt unavailable or inconsistent', identity);
        }
        if (attempt.idempotencyKey !== identity.idempotencyKey)
            return unknown('Output belongs to a different original attempt', identity);
        if (attempt.submissionFingerprint !== identity.submissionFingerprint)
            return freeze({ status: 'IDEMPOTENCY_FINGERPRINT_CONFLICT',
        ...
        if (attempt.intentId !== intentId)
            return unknown('Original intent mismatch', identity);
    ...
    function submit(submission) {
        ...
        try {
            ...
            if (entries(output, 3).length !== 0)
                return Promise.resolve(retained(identity, stable.intentId));
            if (!equal(selectInput(input), selected))
                return Promise.resolve(unknown('Selected inputs changed before intake', identity));
            ...
            syncDirectory(packet);
            for (const item of selected) {
                const bytes = readRegular(join(input, item.name), FILE_BYTES);
                if (bytes.length !== item.bytes || sha256(bytes) !== item.sha256)
                    fail('Selected input changed during intake');
            ...
            if (verified.status !== 'PACKET_VERIFIED')
                return Promise.resolve(unknown(verified.reason, identity));
            ...
            return Promise.resolve(unknown(error instanceof Error ? error.message : 'Packet creation uncertain', identity));
        }
    }
