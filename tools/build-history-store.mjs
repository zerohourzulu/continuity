// Private H03 build. Never replaces a published SDK or selects an npm release.
...
const destination=join(root,'sdk/history-store');
...
try{
 ...
 writeFileSync(join(scratch,'BUILD-PROVENANCE.json'),JSON.stringify({schemaVersion:'continuity-history-store-build/1',version:'0.3.0-h03.1',baselineArchiveSha256:'44aa6d016eb8f235906d3abedf1b70d5d3424de1d66bbb753d022169b63e4f0c',compiler:compiler.version,compilerConfigSha256:sha(readFileSync(join(root,'tools/sdk-build/tsconfig-core-0.3.json'))),sources,outputs},null,2)+'\n');
 if(check){if(JSON.stringify(files(scratch))!==JSON.stringify(files(destination)))throw Error('File membership differs');for(const path of files(scratch))if(!readFileSync(join(scratch,path)).equals(readFileSync(join(destination,path))))throw Error('Build drift '+path);console.log('Private H03 reproducible build matches');}
 else{rmSync(destination,{recursive:true,force:true});mkdirSync(destination,{recursive:true});cpSync(scratch,destination,{recursive:true});console.log('Private H03 JavaScript and declarations built; public SDK unchanged');}
