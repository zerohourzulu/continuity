...
if (args.length > 1 || args.some(x=>!['--check','--pack'].includes(x))) throw Error('Use: node tools/build-core-0.3.mjs [--check|--pack]');
const scratch = mkdtempSync(join(tmpdir(),'continuity-core03-build-'));
try {
  ...
  const manifest = {name:'@continuity/core',version:'0.3.0-preview.5',private:true,type:'module',license:'Apache-2.0',description:'Continuity 0.3 signed local API and protected evidence tool preview',engines:{node:'^22.18.0 || ^24.0.0 || ^26.0.0'},exports,files:['dist','LICENSE','NOTICE','LICENSING.md','THIRD-PARTY-NOTICES.md','README.md','BUILD-PROVENANCE.json']};
  ...
  writeFileSync(join(scratch,'BUILD-PROVENANCE.json'),JSON.stringify({schemaVersion:'continuity-sdk-build/1',coreVersion:'0.2.2',apiVersion:'0.3.0-preview.5',compiler:compiler.version,compilerConfigSha256:sha(readFileSync(join(root,'tools/sdk-build/tsconfig-core-0.3.json'))),sources,outputs},null,2)+'\n');
