# Private D02 installed consumer

After the existing D02 SDK builder has completed, run with a local Node 24 binary:

```sh
node tools/duty-build/verify-installed.mjs /absolute/log-directory /absolute/node22/bin/node
```
...
exact activation permission, activates, inspects, and retries. A second process
...
TypeScript and library checking enabled, including three expected rejections.
