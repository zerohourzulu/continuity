// Private D02 build. Never replaces a published SDK or selects an npm release.
...
const destination=join(root,'sdk/duty-activation');
...
try{
 ...
 writeFileSync(join(scratch,'BUILD-PROVENANCE.json'),JSON.stringify({schemaVersion:'continuity-duty-activation-build/1',version:'0.3.0-d02.1',baselineArchiveSha256:'4503756472b2addbb67f96ba69f7aae95740c7350a7a03bdbda9357d0e16451c',compiler:compiler.version,compilerConfigSha256:sha(readFileSync(join(root,'tools/sdk-build/tsconfig-core-0.3.json'))),sources,outputs},null,2)+'\n');
 if(check){if(JSON.stringify(files(scratch))!==JSON.stringify(files(destination)))throw Error('File membership differs');for(const path of files(scratch))if(!readFileSync(join(scratch,path)).equals(readFileSync(join(destination,path))))throw Error('Build drift '+path);console.log('Private D02 reproducible build matches');}
 else{rmSync(destination,{recursive:true,force:true});mkdirSync(destination,{recursive:true});cpSync(scratch,destination,{recursive:true});console.log('Private D02 JavaScript and declarations built; public SDK unchanged');}
