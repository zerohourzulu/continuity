// Private D03 build. Never replaces a published SDK or selects an npm release.
...
const destination=join(root,'sdk/duty-disposition');
...
try{
 ...
 writeFileSync(join(scratch,'BUILD-PROVENANCE.json'),JSON.stringify({schemaVersion:'continuity-duty-disposition-build/1',version:'0.3.0-d03.1',baselineArchiveSha256:'a17507b9b490e7c804ba8de43823b29f4439a7f34b303ef7e890194d5e472f6b',compiler:compiler.version,compilerConfigSha256:sha(readFileSync(join(root,'tools/sdk-build/tsconfig-core-0.3.json'))),sources,outputs},null,2)+'\n');
 if(check){if(JSON.stringify(files(scratch))!==JSON.stringify(files(destination)))throw Error('File membership differs');for(const path of files(scratch))if(!readFileSync(join(scratch,path)).equals(readFileSync(join(destination,path))))throw Error('Build drift '+path);console.log('Private D03 reproducible build matches');}
 else{rmSync(destination,{recursive:true,force:true});mkdirSync(destination,{recursive:true});cpSync(scratch,destination,{recursive:true});console.log('Private D03 JavaScript and declarations built; public SDK unchanged');}
