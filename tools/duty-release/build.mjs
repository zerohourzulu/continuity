...
const out=join(root,'sdk/duty-release'),scratch=mkdtempSync(join(tmpdir(),'continuity-release-build-'));
...
const base=(part,extra)=>({name:'@ramex-labs/'+names[part],version:versions[part],private:true,type:'module',license:'Apache-2.0',repository:{type:'git',url:'https://github.com/zerohourzulu/continuity.git'},publishConfig:{access:'public',tag:'preview',registry:'https://registry.npmjs.org'},...extra});
const provenance=(part,inputRoots)=>{const dir=join(scratch,part);json(join(dir,'BUILD-PROVENANCE.json'),{format:'continuity-coordinated-build/1',part,version:versions[part],compiler:'5.9.3',buildInputs:['tools/duty-release/build.mjs','tools/duty-release/npm.mjs','tools/duty-release/versions.json','tools/sdk-build/tsconfig-core-0.3.json','docs/releases/'+part+'-'+versions[part]+'.md'].map(path=>({path,sha256:sha(readFileSync(join(root,path)))})),inputs:inputRoots.flatMap(path=>{const p=join(root,path);return files(p).map(f=>({path:path+'/'+f,sha256:sha(readFileSync(join(p,f)))}));}),outputs:files(dir).map(path=>({path,sha256:sha(readFileSync(join(dir,path)))}))});};
try{
