...
for(const file of ['integrations/protected-evidence-mcp/package-lock.json','integrations/langchain-evidence/package-lock.json','integrations/policy-composition/package-lock.json','packages/mcp-gateway/npm-shrinkwrap.json','tools/duty-release/consumer-lock.json']) {
 ...
 for(const [name,row] of Object.entries(lock.packages)) {
  ...
  const path=file==='tools/duty-release/consumer-lock.json'?resolve(root,'sdk/duty-release',row.resolved.slice('file:packages/'.length)):resolve(root,dirname(file),row.resolved.slice(5));
