...
const source=resolve(process.argv[2]),artifacts=join(source,'sdk/protected-remote-artifacts'),out=join(artifacts,'npm-package'),cache=join(tmpdir(),'continuity-remote-pack-cache');
...
const selected=['wire.mjs','protected-evidence.mjs','destination.mjs','client.mjs','durable-store.mjs','validation.mjs','executor.mjs','recovery.mjs','langchain.mjs'];
...
for(const name of ['index.mjs','index.d.mts','langchain.d.mts','local.mjs','local.d.mts','runtime.mjs','runtime.d.mts','attempts.mjs','attempts.d.mts'])cpSync(join(here,['index.mjs','index.d.mts'].includes(name)?'protected-templates':'templates',name),join(remote,name));
...
const manifest={name:'@ramex-labs/continuity-remote',version:'0.3.0-h05.1',private:true,repository:{type:'git',url:'https://github.com/zerohourzulu/continuity.git'},type:'module',license:'Apache-2.0',
...
cpSync(join(source,'packages/remote-tools/package-docs/0.3.0-h05.1.md'),join(out,'README.md'));
...
const provenance={kind:'continuity-remote-shared-core-build/1',compiler:compiler.version,coreDependency:{name:'@ramex-labs/continuity',version:'0.3.0-h04.1',archiveSha256:sha(readFileSync(join(source,'sdk/history-packages/ramex-labs-continuity-0.3.0-h04.1.tgz')))},
  ...
  buildInputs:['tools/remote-build/build-protected.mjs',...walk(join(source,'tools/remote-build/protected-templates')).map(path=>'tools/remote-build/protected-templates/'+path),...walk(join(source,'tools/remote-build/templates')).map(path=>'tools/remote-build/templates/'+path),'tools/sdk-build/tsconfig-core-0.3.json','tools/sdk-build/package-lock.json'].map(path=>({path,sha256:sha(readFileSync(join(source,path)))})),
