...
run([npm,'ci',...flags]);
const archive=join(source,'sdk/investigation-release/ramex-labs-continuity-remote-0.3.0-preview.5.tgz');
run([npm,'install',...flags,join(source,'sdk/investigation-release/ramex-labs-continuity-0.3.0-preview.10.tgz'),archive]);
...
if(process.env.CONTINUITY_CONSUMER_RECORD)writeFileSync(process.env.CONTINUITY_CONSUMER_RECORD,JSON.stringify({directory:consumer,node:process.version,package:'@ramex-labs/continuity-remote@0.3.0-preview.5',checks:['strict public types','signed walkthrough','native installed invocation']},null,2)+'\n');
