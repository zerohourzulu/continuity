...
const root=resolve(scriptDirectory,'../..'),sdk=join(root,'sdk/duty-disposition');
const [logArgument,node22Argument]=process.argv.slice(2);
if(!logArgument||process.argv.length>4) throw new Error('Usage: node24 tools/disposition-build/verify-installed.mjs LOG_DIRECTORY [NODE22]');
...
const scratch=realpathSync(mkdtempSync(join(tmpdir(),'continuity-d03-installed-')));
...
writeFileSync(join(consumer,'package.json'),JSON.stringify({name:'d03-installed-consumer',private:true,type:'module'}));
...
assert.equal(metadata.version,'0.3.0-d03.1');assert.equal(metadata.private,true);
