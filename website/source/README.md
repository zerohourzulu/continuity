...
|---|---|
| Understand the idea without installing anything | [Five-minute explanation](docs/START-HERE.md) |
| Run it and see the result | The commands below, then [the walkthrough](docs/TUTORIAL.md) |
| Inspect recorded Linux enforcement evidence | [Operator guide](docs/OPERATOR.md) |
| Integrate a deterministic authority check | [Developer guide](docs/DEVELOPER.md) and [working example](examples/check-authority.mjs) |
| Try native Linux boundary mechanics | [Optional disposable Linux lab](docs/LINUX-LAB.md) |
...
Only local test files are written. No document is sent to another person or system. See [expected stages and files](docs/TUTORIAL.md). Existing case names are refused; choose a new name to repeat.
