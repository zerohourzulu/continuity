...
import { hashCanonical, immutableProtocolValue, immutableProtocolInput, replayPortable, PORTABLE_REPLAY_VERSION } from "../core/index.ts";
...
const encode = (v: unknown): unknown => {
  ...
  if (typeof v === "object") return ["object", Object.entries(v).map(([k, x]) => [k, encode(x)])];
  if (["string", "boolean", "number"].includes(typeof v)) return [typeof v, v];
  throw new Error("Unsupported stored value.");
};
const decode = (v: unknown, depth = 0): unknown => {
  if (depth > 128 || !Array.isArray(v)) throw new Error("Invalid stored encoding.");
  const [tag, body] = v;
  if (tag === "null" && v.length === 1) return null;
  if (v.length !== 2) throw new Error("Invalid stored encoding.");
  ...
  if (tag === "number" && typeof body === "number" && Number.isSafeInteger(body) && body >= 0) return body;
  if (tag === "boolean" && typeof body === "boolean") return body;
  if (tag === "array" && Array.isArray(body)) return body.map(x => decode(x, depth + 1));
  if (tag === "object" && Array.isArray(body)) {
    const result: Record<string, unknown> = Object.create(null);
    for (const entry of body) {
      if (!Array.isArray(entry) || entry.length !== 2 || typeof entry[0] !== "string" || Object.hasOwn(result, entry[0])) throw new Error("Invalid stored object.");
...
const replay = (events: readonly PortableCanonicalEvent[]): PortableHistoryHead => {
  const result = replayPortable({ operationVersion: PORTABLE_REPLAY_VERSION, events });
  if (result.status !== "ACCEPTED") throw new Error(`Portable store history rejected: ${result.code}.`);
  return result.head;
};
const rejectDanglingLink = (path: string): void => {
  if (!existsSync(path)) {
    try { if (lstatSync(path).isSymbolicLink()) throw new Error("Dangling store symlinks are unsupported."); }
    catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
  }
};
const chainHash = (previousHash: string | null, event: PortableCanonicalEvent) => hashCanonical({ version: FORMAT, previousHash, event });

export class PortableStoreConflictError extends Error {
  declare readonly observedHead?: PortableHistoryHead;
  constructor(observedHead?: PortableHistoryHead) {
    ...
    if (observedHead !== undefined) this.observedHead = immutableProtocolValue(observedHead);
  }
}
...
export class PortableFileEventStore {
  ...
  #checkPath(): void {
    ...
    if (existsSync(this.path)) {
      ...
      if (!info.isFile() || info.nlink !== 1 || realpathSync(this.path) !== this.path) throw new Error("Portable store requires a regular, non-aliased file.");
    }
  }
  readAll(): readonly PortableCanonicalEvent[] {
    this.#checkPath();
    ...
    try {
      ...
      if (size > MAX_FILE_BYTES) throw new Error("Portable store file limit exceeded.");
      const bytes = Buffer.alloc(size);
      let offset = 0;
      while (offset < size) {
        const count = readSync(fd, bytes, offset, size - offset, offset);
        if (!count) throw new Error("Portable store truncated during read.");
        offset += count;
      }
    ...
    const events: PortableCanonicalEvent[] = [];
    let previousHash: string | null = null;
    for (const line of contents.slice(0, -1).split("\n")) {
      const record = JSON.parse(line);
      if (record === null || typeof record !== "object" || Object.keys(record).sort().join(",") !== "event,previousHash,recordHash,version" || record.version !== FORMAT || record.previousHash !== previousHash) throw new Error("Portable store format or hash chain mismatch.");
      const event = decode(record.event) as PortableCanonicalEvent;
      if (JSON.stringify(encode(event)) !== JSON.stringify(record.event)) throw new Error("Noncanonical store encoding.");
      const hash = chainHash(previousHash, event);
      if (record.recordHash !== hash) throw new Error("Portable store integrity check failed.");
      events.push(event);
      previousHash = hash;
    }
  ...
  #append(event: PortableCanonicalEvent, events: readonly PortableCanonicalEvent[]): PortableHistoryHead {
    ...
    let previousHash: string | null = null;
    for (const item of events) previousHash = chainHash(previousHash, item);
    ...
    try {
      if (fstatSync(fd).size + bytes.length > MAX_FILE_BYTES) throw new Error("Portable store file limit exceeded.");
      let offset = 0;
      while (offset < bytes.length) {
        const count = writeSync(fd, bytes, offset, bytes.length - offset);
        if (count <= 0) throw new Error("Portable store made no write progress.");
        offset += count;
      }
      fsyncSync(fd);
  ...
  withExclusiveWriter<T>(callback: (writer: PortableStoreWriter) => T): T {
    ...
    catch (error) { if ((error as NodeJS.ErrnoException).code === "EEXIST") throw new PortableStoreBusyError(); throw error; }
    let active = true;
    let operating = false;
    const check = () => { if (operating) throw new Error("Portable writer operation is not reentrant."); if (!active) throw new Error("Portable store writer capability expired."); };
    const writer: PortableStoreWriter = Object.freeze({
      ...
      appendAtExpectedHead: (event: PortableCanonicalEvent, expected: PortableHistoryHead) => {
        ...
        try {
          ...
          if (observed === undefined || stableExpected.hash !== observed.hash || stableExpected.position !== observed.position || stableExpected.canonicalTime !== observed.canonicalTime) throw new PortableStoreConflictError(observed);
          return this.#append(event, events);
        } finally { operating = false; }
      },
    });
    try { return callback(writer); }
    finally { active = false; try { closeSync(fd); } finally { unlinkSync(lock); } }
  }
  ...
  appendAll(input: readonly PortableCanonicalEvent[]): void {
    ...
    this.withExclusiveWriter(writer => {
      ...
      if (stable.length > 0) replay([...events, ...stable]);
      for (const event of stable) { this.#append(event, events); events = [...events, event]; }
    });
  }
  appendAtExpectedHead(event: PortableCanonicalEvent, expected: PortableHistoryHead): PortableHistoryHead {
