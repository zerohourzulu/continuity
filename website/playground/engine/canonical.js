...
import { HostTypeError, arrayIncludes, arrayIsArray, arrayJoin, arrayPop, arrayPush, arraySort, bigintFrom, bigintToString, cloneKeccak256State, copyArray, createKeccak256State, createSet, createWeakMap, createWeakSet, finalizeKeccak256State, hostObjectPrototype, keccak256Bytes, mathCeil, mathMax, mathMin, numberFrom, numberIsFinite, numberIsSafeInteger, numberToString, objectCreate, objectDefineDataProperty, objectFreeze, objectHasOwn, objectIs, reflectApply, reflectGetOwnPropertyDescriptor, reflectGetPrototypeOf, reflectOwnKeys, regExpTest, setAdd, setHas, stringCharCodeAt, stringFrom, stringPadStart, stringStartsWith, utf8Encode, uint8ArrayLength, updateKeccak256State, weakMapGet, weakMapHas, weakMapSet, weakSetAdd, weakSetDelete, weakSetHas, } from "./host-intrinsics.js";
...
const REPLAY_VISITOR_PENDING_FIXED_BYTES = uint8ArrayLength(UTF8_ENCODER.encode(`],"operationVersion":"${REPLAY_OPERATION_VERSION}"}`));
...
const protocolStringByteLength = (value, location, maximum = MAX_PROTOCOL_STRING_UTF8_BYTES) => {
    let bytes = 0;
    for (let index = 0; index < value.length; index += 1) {
        const unit = stringCharCodeAt(value, index);
        if (unit <= 0x7f) {
        ...
        if (bytes > maximum) {
            throw canonicalNamedLimitError(`Canonical string at ${location} exceeds the ${maximum}-byte ProtocolString bound.`, location, maximum);
        }
    }
    return bytes;
};
/** Unsigned UTF-8 lexicographic protocol-string order, without normalization. */
export const compareProtocolStrings = (left, right) => {
...
const escapedString = (value, location) => {
...
const brandCanonicalCaptureLimitError = (error, location, limit) => {
    const metadata = {};
...
const canonicalCaptureLimitError = (location, limit) => brandCanonicalCaptureLimitError(new TypeError(`Canonical value at ${location} exceeds the ${limit}-byte canonical capture bound.`), location, limit);
const canonicalNamedLimitError = (message, location, limit) => brandCanonicalCaptureLimitError(new TypeError(message), location, limit);
/** Tests only the module-private identity and exact metadata of a limit failure. */
export const isCanonicalCaptureLimitError = (error, location, limit) => {
...
export const getCanonicalCaptureLimitErrorMetadata = (error) => error !== null && typeof error === "object"
    ? weakMapGet(CANONICAL_CAPTURE_LIMIT_ERRORS, error)
    : undefined;
const CANONICAL_REPLAY_EVENT_CAPTURES = createWeakSet();
/** Tests module-private provenance for one incrementally captured replay event. */
export const isCapturedCanonicalReplayEvent = (value) => value !== null &&
    typeof value === "object" &&
    weakSetHas(CANONICAL_REPLAY_EVENT_CAPTURES, value);
const ADAPTER_EVIDENCE_CAPTURE_ROLE = Object.freeze({ kind: "ADAPTER_EVIDENCE" });
const NORMAL_CAPTURE_ROLE = Object.freeze({ kind: "NORMAL" });
const REPLAY_ROOT_CAPTURE_ROLE = Object.freeze({ kind: "REPLAY_ROOT" });
const REPLAY_EVENTS_CAPTURE_ROLE = Object.freeze({ kind: "REPLAY_EVENTS" });
const REPLAY_DESIGNATED_SCALAR_CAPTURE_ROLE = Object.freeze({
    kind: "REPLAY_DESIGNATED_SCALAR",
});
const AUTHORITY_IDENTIFIER_CAPTURE_ROLE = Object.freeze({
    kind: "REPLAY_DESIGNATED_SCALAR",
    maximumStringBytes: MAX_AUTHORITY_IDENTIFIER_UTF8_BYTES,
});
const RAW_AUTHORITY_AMOUNT_CAPTURE_ROLE = Object.freeze({
    kind: "REPLAY_DESIGNATED_SCALAR",
    enforceBigIntBound: true,
});
const portableAuthorityRole = (state) => Object.freeze({ kind: "PORTABLE_AUTHORITY", state });
const portableAuthorityRootRole = (kind) => portableAuthorityRole(kind === "AUTHORITY_PATH_EVALUATION"
    ? "RAW_ROOT"
    : kind === "AUTHORIZE"
        ? "AUTHORIZE_ROOT"
        : "ADMISSION_ROOT");
const portableAuthorityRecordChildRole = (state, key) => {
    switch (state) {
        case "RAW_ROOT":
            if (key === "scope")
                return portableAuthorityRole("RAW_SCOPE");
            if (key === "request")
                return portableAuthorityRole("ACTION_REQUEST");
            if (key === "permissions")
                return portableAuthorityRole("PERMISSION_LIST");
            if (key === "prohibitions")
                return portableAuthorityRole("PROHIBITION_LIST");
            ...
            if (key === "usage")
                return portableAuthorityRole("AUTHORITY_USAGE_LIST");
            return NORMAL_CAPTURE_ROLE;
        case "AUTHORIZE_ROOT":
            if (key === "events")
                return REPLAY_EVENTS_CAPTURE_ROLE;
            if (key === "expectedHistoryHead") {
                return portableAuthorityRole("HISTORY_HEAD");
            }
            if (key === "domain")
                return portableAuthorityRole("DOMAIN");
        ...
        case "AUTHORITY_USAGE":
            if (key === "authorityId")
                return AUTHORITY_IDENTIFIER_CAPTURE_ROLE;
            if (key === "admittedCumulativeAmount")
                return RAW_AUTHORITY_AMOUNT_CAPTURE_ROLE;
...
const portableAuthorityArrayChildRole = (state) => {
    switch (state) {
        case "RECOGNIZED_ROOT_LIST":
...
const reserveReceiptEventOccurrences = (context, count, location) => {
    ...
    if (count > MAX_CANONICAL_LIST_MEMBERS - counter.used) {
        ...
        throw canonicalNamedLimitError("Receipt operation exceeds the 4096-event occurrence bound.", location, MAX_CANONICAL_LIST_MEMBERS);
    }
    counter.used += count;
};
const authorityListMaximum = (role) => {
    if (role.kind !== "PORTABLE_AUTHORITY")
        return MAX_CANONICAL_LIST_MEMBERS;
    switch (role.state) {
...
const reserveRawAuthorityOccurrences = (context, count, location, role) => {
    const counter = context.rawAuthorityOccurrences;
    if (counter === undefined || role.kind !== "PORTABLE_AUTHORITY" ||
        (role.state !== "PERMISSION_LIST" && role.state !== "PROHIBITION_LIST"))
        return;
    if (count > MAX_RAW_AUTHORITY_OCCURRENCES - counter.used) {
        counter.used = MAX_RAW_AUTHORITY_OCCURRENCES + 1;
        throw canonicalNamedLimitError(`Raw authority input at ${location} exceeds the ${MAX_RAW_AUTHORITY_OCCURRENCES}-authority combined bound.`, location, MAX_RAW_AUTHORITY_OCCURRENCES);
    }
    counter.used += count;
};
/** Named limits do not promote wrong-type scalar values to semantic validity. */
const enforceAuthorityScalarLimits = (value, role, location) => {
    if (role.kind !== "REPLAY_DESIGNATED_SCALAR")
        return;
    const maximum = role.maximumStringBytes;
    if (typeof value === "string" && maximum !== undefined) {
        if (value.length > maximum) {
            throw canonicalNamedLimitError(`Canonical Identifier at ${location} exceeds the ${maximum}-byte bound.`, location, maximum);
        }
        // A bounded malformed string keeps its later INVALID_REQUEST/field routing.
        if (isWellFormedUnicode(value))
            protocolStringByteLength(value, location, maximum);
    }
...
const enforceDesignatedScalarHostKind = (value, role, location) => {
    if (role.kind !== "REPLAY_DESIGNATED_SCALAR")
        return;
    if (value === undefined) {
...
const enforceCapturedMemberAuthorityRole = (container, key, value, context, role, location) => {
    if (role.kind === "REPLAY_DESIGNATED_SCALAR") {
...
const enforceMemoizedAuthorityRole = (value, context, role, location) => {
    ...
    if (Array.isArray(value)) {
        ...
        if (value.length > maximum) {
            throw canonicalNamedLimitError(`Canonical array at ${location} exceeds the ${maximum}-member bound.`, location, maximum);
        }
    ...
    if (keys === undefined)
        throw new TypeError("Authority alias lost captured record-key metadata.");
    for (let index = 0; index < keys.length; index += 1) {
        const key = keys[index];
        const member = value[key];
        if (member !== undefined) {
            enforceCapturedMemberAuthorityRole(value, key, member, context, portableAuthorityRecordChildRole(role.state, key), `${location}.${key}`);
        }
    }
};
const REPLAY_CAPTURE_VISITOR_STOPS = createWeakMap();
const REPLAY_CAPTURE_VISITOR_THROWS = createWeakMap();
const replayCaptureVisitorToken = () => Object.freeze(Object.create(null));
const reserveBytes = (context, amount) => {
...
const reserveText = (context, text) => {
    const bytes = uint8ArrayLength(UTF8_ENCODER.encode(text));
    reserveBytes(context, bytes);
    return bytes;
};
const textRope = (text, utf8Bytes) => Object.freeze({ kind: "TEXT", text, utf8Bytes });
const sequenceRope = (parts) => Object.freeze({ kind: "SEQUENCE", parts: Object.freeze(parts) });
...
const MAX_CANONICAL_OUTPUT_SEGMENTS = Math.ceil(MAX_CANONICAL_UTF8_BYTES / CANONICAL_OUTPUT_BUFFER_BYTES);
const materializeCanonicalRope = (root, expectedBytes) => {
    const stack = [root];
    let bufferedParts = [];
    let bufferedBytes = 0;
    ...
    while (stack.length > 0) {
        ...
        if (bufferedBytes >= CANONICAL_OUTPUT_BUFFER_BYTES)
            flush();
    }
    flush();
    if (emittedBytes !== expectedBytes) {
...
const canonicalScalarNode = (value, text, context) => {
...
const noncanonicalScalarNode = (value, context) => {
...
const bigintDecimal = (value, location) => {
    if (value < -MAX_PROTOCOL_BIGINT || value > MAX_PROTOCOL_BIGINT) {
        throw canonicalNamedLimitError(`Canonical BigInt at ${location} exceeds the 256-bit protocol bound.`, location, 256);
    }
    return bigintToString(value, 10);
};
const parseMarkerBigInt = (value, location) => {
...
const readPrototype = (value, location) => {
    try {
...
const readOwnKeys = (value, location) => {
    try {
...
const readDescriptor = (value, key, location) => {
    let descriptor;
    try {
...
const descriptorValue = (descriptor, receiver, location) => {
...
const capturedDescriptorValue = (context, descriptor, receiver, location, adapterField = false) => {
    if (!Object.hasOwn(descriptor, "value")) {
...
const canonicalArrayIndex = (key) => {
    if (key === "0")
        return 0;
    if (!regExpTest(/^[1-9][0-9]*$/, key))
        return undefined;
...
const reuseNode = (node, context, containerDepth, location) => {
    if (containerDepth + node.intrinsicDepth > MAX_CANONICAL_DEPTH) {
        throw canonicalNamedLimitError(`Canonical value at ${location} exceeds nesting depth ${MAX_CANONICAL_DEPTH}.`, location, MAX_CANONICAL_DEPTH);
    }
    reserveBytes(context, node.canonicalBytes);
    return node;
};
const enforceMemoizedReplayEventDataBudget = (node, context, eventIndex) => {
    if (node.value === null ||
    ...
    if (dataBytes !== undefined &&
        ...
        throw canonicalCaptureLimitError(`$.events[${eventIndex}].data`, MAX_REPLAY_EVENT_DATA_UTF8_BYTES);
    }
};
/** Reusing a query envelope preserves only its three designated scalar slots. */
const enforceMemoizedQueryEventGrammar = (node, context) => {
    ...
    for (const key of ["id", "type", "timestamp"]) {
        ...
        if (value !== undefined) {
            enforceCapturedMemberAuthorityRole(node.value, key, value, context, REPLAY_DESIGNATED_SCALAR_CAPTURE_ROLE, `$.events[].${key}`);
        }
    }
    if (node.canonicalRope !== null)
        return;
    const keys = weakMapGet(context.recordKeysByCapturedValue, node.value);
    if (keys === undefined)
        throw new TypeError("Query alias is not a canonical event envelope.");
    for (let index = 0; index < keys.length; index += 1) {
        const key = keys[index];
        ...
        if (invalid)
            throw new TypeError("Query alias introduces a classified scalar into strict event data.");
    }
};
const CANONICAL_REPLAY_BASE_FAILURES = createWeakMap();
const CANONICAL_REPLAY_SCHEMA_FAILURES = createWeakSet();
const canonicalReplaySchemaFailure = (message) => {
...
export const isCanonicalReplaySchemaFailure = (error) => error !== null &&
    typeof error === "object" &&
    weakSetHas(CANONICAL_REPLAY_SCHEMA_FAILURES, error);
/** @internal Read-only provenance for a replay member capture/base failure. */
export const getCanonicalReplayBaseFailureMetadata = (error) => error !== null && typeof error === "object"
    ? weakMapGet(CANONICAL_REPLAY_BASE_FAILURES, error)?.metadata
    : undefined;
const replayBaseFailureForContext = (error, context) => {
    if (error === null || typeof error !== "object")
        return undefined;
...
const annotateReplayBaseFailure = (error, context, metadata) => {
    ...
    if (replayBaseFailureForContext(throwable, context) === undefined) {
        ...
        Object.defineDataProperty(branded, "captureToken", context.replayFailureToken, false, true);
...
const replayBaseFailureMetadata = (eventPosition, code, eventId) => {
    const metadata = {};
...
const isReplayBaseProtocolString = (value, maximumBytes) => {
...
const failReplayBaseNode = (node, context, eventPosition, code, includeEventId = false) => {
    const error = canonicalReplaySchemaFailure(code === "EVENT_DATA_INVALID"
    ...
    return annotateReplayBaseFailure(error, context, replayBaseFailureMetadata(eventPosition, code, eventId));
};
const validateCapturedReplayBaseNode = (node, context, eventPosition) => {
    if (node.value === null ||
        typeof node.value !== "object" ||
        Array.isArray(node.value)) {
    ...
    const refineScalar = (key, role) => {
        try {
            enforceCapturedMemberAuthorityRole(record, key, record[key], context, role, `$.events[].${key}`);
        }
        catch (error) {
            return annotateReplayBaseFailure(error, context, replayBaseFailureMetadata(eventPosition, "EVENT_ENVELOPE_INVALID", key === "id" ? undefined : replayBaseEventId(node)));
        }
    };
...
const invokeReplayCaptureVisitor = (node, context, eventPosition) => {
    if (context.replayVisitor === undefined)
        return;
    const capturedRecordKeys = Object.freeze((candidate) => candidate !== null &&
        (typeof candidate === "object" || typeof candidate === "function")
        ? weakMapGet(context.recordKeysByCapturedValue, candidate)
        : undefined);
    const capturedCanonicalBytes = Object.freeze((candidate) => candidate !== null &&
        (typeof candidate === "object" || typeof candidate === "function")
...
const captureReplayEvent = (value, context, containerDepth, location, eventPosition) => {
    ...
    try {
        ...
        if (rawKeys.length > MAX_CANONICAL_RECORD_FIELDS) {
            throw canonicalNamedLimitError(`Canonical record at ${location} exceeds the ${MAX_CANONICAL_RECORD_FIELDS}-field bound.`, location, MAX_CANONICAL_RECORD_FIELDS);
        }
        const keys = [];
        ...
        const membership = createSet();
        for (let keyIndex = 0; keyIndex < keys.length; keyIndex += 1) {
            setAdd(membership, keys[keyIndex]);
        }
        const exactMembership = keys.length === 4 &&
            setHas(membership, "id") &&
            setHas(membership, "type") &&
            setHas(membership, "timestamp") &&
            setHas(membership, "data");
        if (!exactMembership) {
            throw canonicalReplaySchemaFailure(`Canonical replay event at ${location} has an invalid envelope.`);
        }
        weakSetAdd(context.active, value);
        const startBytes = context.budget.used;
        try {
            const captureEnvelopeScalar = (key) => {
                if (!setHas(membership, key))
                    return undefined;
                ...
                return captureNode(field, context, depth, `${location}.${key}`, key === "id" ? AUTHORITY_IDENTIFIER_CAPTURE_ROLE : REPLAY_DESIGNATED_SCALAR_CAPTURE_ROLE);
            };
            ...
            if (idNode === undefined ||
                ...
                throw canonicalReplaySchemaFailure(`Canonical replay event at ${location} has an invalid id.`);
            }
            ...
            if (typeNode === undefined ||
                ...
                throw canonicalReplaySchemaFailure(`Canonical replay event at ${location} has an invalid type.`);
            }
            const timestampNode = captureEnvelopeScalar("timestamp");
            if (timestampNode === undefined ||
                ...
                throw canonicalReplaySchemaFailure(`Canonical replay event at ${location} has an invalid timestamp.`);
            }
            ...
            if (data === null ||
                typeof data !== "object" ||
                Array.isArray(data)) {
                throw canonicalReplaySchemaFailure(`Canonical replay event data at ${location}.data is not a plain record.`);
            }
            ...
            if (memoizedData === undefined && !weakSetHas(context.active, data)) {
                ...
                if (dataPrototype !== Object.prototype && dataPrototype !== null) {
                    throw new TypeError(`Canonical replay event data at ${location}.data is not a plain record.`);
                }
                weakMapSet(context.preobservedPrototypes, data, dataPrototype);
            }
            else if (memoizedData !== undefined &&
            ...
            const dataNode = captureNode(data, context, depth, `${location}.data`, Object.freeze({ kind: "REPLAY_EVENT_DATA", index: eventPosition }));
            ...
            Object.defineDataProperty(immutable, "timestamp", timestampNode.value, false, true);
            ...
            appendText('{"data":');
            if (dataNode.canonicalRope !== null)
                arrayPush(ropeParts, dataNode.canonicalRope);
            appendText(',"id":');
            if (idNode.canonicalRope !== null)
                arrayPush(ropeParts, idNode.canonicalRope);
            appendText(',"timestamp":');
            if (timestampNode.canonicalRope !== null)
                arrayPush(ropeParts, timestampNode.canonicalRope);
            appendText(',"type":');
            if (typeNode.canonicalRope !== null)
                arrayPush(ropeParts, typeNode.canonicalRope);
            appendText("}");
            const hasCanonicalRope = dataNode.canonicalRope !== null &&
            ...
            weakMapSet(context.recordKeysByCapturedValue, immutable, Object.freeze(["data", "id", "timestamp", "type"]));
        ...
        if (replayBaseFailureForContext(error, context) !== undefined)
            throw error;
        return annotateReplayBaseFailure(error, context, replayBaseFailureMetadata(eventPosition, "EVENT_ENVELOPE_INVALID", eventId));
    }
};
const captureArray = (value, context, containerDepth, location, role) => {
    const depth = containerDepth + 1;
    if (depth > MAX_CANONICAL_DEPTH) {
        throw canonicalNamedLimitError(`Canonical value at ${location} exceeds nesting depth ${MAX_CANONICAL_DEPTH}.`, location, MAX_CANONICAL_DEPTH);
    }
    ...
    for (let rawKeyIndex = 0; rawKeyIndex < rawKeys.length; rawKeyIndex += 1) {
        ...
        if (rawKey === "length") {
            if (hasLength)
                throw new TypeError(`Canonical array at ${location} has duplicate length metadata.`);
            hasLength = true;
            continue;
        }
    ...
    if (indexKeys.length !== length) {
        throw new TypeError(`Canonical array at ${location} is sparse or exceeds the ${maximum}-member bound.`);
    }
    ...
    try {
        ...
        for (let index = 0; index < length; index += 1) {
            ...
            try {
                if (index > 0) {
                    reserveBytes(context, 1);
                    if (ropeParts !== null)
                        arrayPush(ropeParts, COMMA_ROPE);
                }
                const descriptor = readDescriptor(value, stringFrom(index), `${location}[${index}]`);
                ...
                const childRole = role.kind === "REPLAY_EVENTS"
                    ...
                    : role.kind === "QUERY_EVENTS"
                        ...
                        : role.kind === "PORTABLE_AUTHORITY"
                            ...
                            : NORMAL_CAPTURE_ROLE;
                child = captureNode(member, context, depth, `${location}[${index}]`, childRole);
                arrayPush(childNodes, child);
                if (member !== null && typeof member === "object" && typeof child.value === "bigint") {
                    normalizedMembers ??= Object.create(null);
                    Object.defineDataProperty(normalizedMembers, stringFrom(index), true, true, true, true);
                }
                maximumChildIntrinsicDepth = Math.max(maximumChildIntrinsicDepth, child.intrinsicDepth);
                if (ropeParts !== null) {
                    if (child.canonicalRope === null)
                        ropeParts = null;
                    else
                        arrayPush(ropeParts, child.canonicalRope);
                }
            }
            catch (error) {
                ...
                const capturedMember = child ?? (member !== null && typeof member === "object"
                    ? weakMapGet(context.memo, member)
                    : undefined);
                const eventId = capturedMember === undefined
                    ? undefined
                    : replayBaseEventId(capturedMember);
                annotateReplayBaseFailure(error, context, replayBaseFailureMetadata(index, "EVENT_ENVELOPE_INVALID", eventId));
            }
            if (role.kind === "REPLAY_EVENTS") {
                if (context.replayVisitor !== undefined &&
        ...
        reserveBytes(context, 1);
        if (ropeParts !== null)
            arrayPush(ropeParts, CLOSE_LIST_ROPE);
...
const captureRecord = (value, context, containerDepth, location, role) => {
    ...
    for (let keyIndex = 0; keyIndex < keys.length; keyIndex += 1) {
        ...
        if (stringStartsWith(key, RESERVED_KEY_PREFIX))
            arrayPush(reservedKeys, key);
    }
    if (reservedKeys.length > 0 &&
        (keys.length !== 1 ||
            reservedKeys.length !== 1 ||
            reservedKeys[0] !== BIGINT_MARKER_KEY)) {
        throw new TypeError(`Canonical record at ${location} uses the reserved ${RESERVED_KEY_PREFIX} key prefix outside the exact BigInt marker shape.`);
    }
    ...
    try {
        reserveBytes(context, 1);
        let ropeParts = [OPEN_RECORD_ROPE];
        const immutable = Object.create(null);
        let normalizedMembers;
        let ordinaryFields = 0;
        let maximumChildIntrinsicDepth = 0;
        let recognizedMarker;
        for (let keyIndex = 0; keyIndex < ordered.length; keyIndex += 1) {
            ...
            if (stringStartsWith(key, RESERVED_KEY_PREFIX)) {
                if (key !== BIGINT_MARKER_KEY ||
                    recognizedMarker !== undefined ||
                    ordinaryFields !== 0) {
                    throw new TypeError(`Canonical record at ${location} uses the reserved ${RESERVED_KEY_PREFIX} key prefix.`);
                }
                recognizedMarker = parseMarkerBigInt(field, `${location}.${key}`);
                const interior = `${escapedString(key, `${location}.{key}`)}:${escapedString(bigintToString(recognizedMarker, 10), `${location}.${key}`)}`;
                const interiorBytes = reserveText(context, interior);
                if (ropeParts !== null)
                    arrayPush(ropeParts, textRope(interior, interiorBytes));
                continue;
            }
            if (field === undefined)
                continue;
            if (recognizedMarker !== undefined) {
                throw new TypeError(`Canonical record at ${location} combines a reserved BigInt marker with ordinary fields.`);
            }
            const depth = containerDepth + 1;
            if (depth > MAX_CANONICAL_DEPTH) {
                throw canonicalNamedLimitError(`Canonical value at ${location} exceeds nesting depth ${MAX_CANONICAL_DEPTH}.`, location, MAX_CANONICAL_DEPTH);
            ...
            reserveBytes(context, 1);
            if (ropeParts !== null)
                arrayPush(ropeParts, textRope(`${keyText}:`, keyBytes + 1));
            const childRole = role.kind === "REPLAY_ROOT" && key === "events"
            ...
            let node;
            try {
                node = captureNode(field, context, depth, `${location}.${key}`, childRole);
            }
            catch (error) {
                if (role.kind === "REPLAY_EVENT" && key === "data") {
                    annotateReplayBaseFailure(error, context, replayBaseFailureMetadata(role.index, "EVENT_ENVELOPE_INVALID"));
                }
                throw error;
            }
            maximumChildIntrinsicDepth = Math.max(maximumChildIntrinsicDepth, node.intrinsicDepth);
            if (ropeParts !== null) {
                if (node.canonicalRope === null)
                    ropeParts = null;
                else
                    arrayPush(ropeParts, node.canonicalRope);
            }
        ...
        reserveBytes(context, 1);
        if (ropeParts !== null)
            arrayPush(ropeParts, CLOSE_RECORD_ROPE);
        const result = {
            value: recognizedMarker ?? Object.freeze(immutable),
            canonicalBytes: context.budget.used - startBytes,
            canonicalRope: ropeParts === null ? null : sequenceRope(ropeParts),
            intrinsicDepth: recognizedMarker === undefined ? 1 + maximumChildIntrinsicDepth : 0,
        };
        if (recognizedMarker === undefined) {
            if (normalizedMembers !== undefined) {
                weakMapSet(context.normalizedObjectMembers, result.value, Object.freeze(normalizedMembers));
            }
            weakMapSet(context.recordKeysByCapturedValue, result.value, Object.freeze(copyArray(ordered)));
            weakMapSet(context.nodeByCapturedValue, result.value, result);
        }
        weakMapSet(context.memo, value, result);
        return result;
    }
...
const captureNode = (value, context, containerDepth, location, role = NORMAL_CAPTURE_ROLE) => {
    ...
    if ((context.accessorReads ?? 0) !== before)
        result.observedAccessor = true;
    if (dataOnly && result.observedAccessor)
        throw new TypeError(`Adapter data alias at ${location} contains an accessor.`);
    return result;
};
const captureNodeBody = (value, context, containerDepth, location, role = NORMAL_CAPTURE_ROLE) => {
    enforceAuthorityScalarLimits(value, role, location);
    enforceDesignatedScalarHostKind(value, role, location);
    if (role.kind === "REPLAY_EVENT" &&
        (value === null || typeof value !== "object" || Array.isArray(value))) {
        throw canonicalReplaySchemaFailure(`Canonical replay event at ${location} is not a plain record.`);
    }
    if (role.kind === "REPLAY_EVENT_DATA" || role.kind === "ADAPTER_EVIDENCE") {
        ...
        if (role.kind === "ADAPTER_EVIDENCE")
            context.adapterDataOnlyDepth = (context.adapterDataOnlyDepth ?? 0) + 1;
        try {
            return captureNode(value, context, containerDepth, location, NORMAL_CAPTURE_ROLE);
        }
        finally {
            if (role.kind === "ADAPTER_EVIDENCE")
                context.adapterDataOnlyDepth = (context.adapterDataOnlyDepth ?? 0) - 1;
    ...
    if (value === null)
        return canonicalScalarNode(null, "null", context);
    if (typeof value === "boolean") {
        return canonicalScalarNode(value, value ? "true" : "false", context);
    }
    if (typeof value === "string") {
        if (context.mode === "STRUCTURAL_PHASE_ONE" ||
            ...
            if (value.length > MAX_PROTOCOL_STRING_UTF8_BYTES) {
                throw canonicalNamedLimitError(`Canonical string at ${location} exceeds the ${MAX_PROTOCOL_STRING_UTF8_BYTES}-byte ProtocolString bound.`, location, MAX_PROTOCOL_STRING_UTF8_BYTES);
            }
            if (!isWellFormedUnicode(value)) {
    ...
    if (typeof value === "number") {
        ...
        if (!valid) {
            ...
            throw new TypeError(`Canonical number at ${location} must be a signed safe integer other than negative zero.`);
        }
    ...
    if (typeof value === "bigint") {
        ...
        return canonicalScalarNode(value, `{"${BIGINT_MARKER_KEY}":"${decimal}"}`, context);
    }
    if (value === undefined) {
        throw new TypeError(`Canonical value at ${location} is undefined.`);
    }
    ...
    if (memoized !== undefined) {
        ...
        if (role.kind === "QUERY_EVENTS") {
            if (!Array.isArray(memoized.value))
                throw new TypeError("Query history must be a list.");
    ...
    if (role.kind === "REPLAY_EVENT") {
        return captureReplayEvent(value, context, containerDepth, location, role.index);
    }
    return Array.isArray(value)
        ? captureArray(value, context, containerDepth, location, role)
        : captureRecord(value, context, containerDepth, location, role);
};
const deepCapture = (value, mode, maxCanonicalBytes, options = {}) => {
    const preobservedRootPrototype = Object.hasOwn(options, "preobservedRootPrototype")
    ...
    const rootRole = Object.hasOwn(options, "rootRole")
        ? options.rootRole
        : undefined;
    const preobservedPrototypes = createWeakMap();
    if (preobservedRootPrototype !== undefined) {
        weakMapSet(preobservedPrototypes, preobservedRootPrototype.value, preobservedRootPrototype.prototype);
    }
    const context = {
        mode,
        budget: { label: "$", limit: maxCanonicalBytes, used: 0 },
        active: createWeakSet(),
        memo: createWeakMap(),
        recordKeysByCapturedValue: createWeakMap(),
        normalizedObjectMembers: createWeakMap(),
        nodeByCapturedValue: createWeakMap(),
        localBudgets: [],
    ...
    const node = captureNode(value, context, ancestorContainerDepth ?? 0, "$", rootRole ?? NORMAL_CAPTURE_ROLE);
    const capturedRecordKeys = Object.freeze((candidate) => candidate !== null &&
        (typeof candidate === "object" || typeof candidate === "function")
        ? weakMapGet(context.recordKeysByCapturedValue, candidate)
        : undefined);
    ...
    return {
        value: node.value,
        canonicalText: node.canonicalRope === null
            ? null
 ...
 * caller-owned data.
 */
export const captureBoundedCanonicalValue = (value, options = {}) => {
...
const CANONICAL_REPLAY_BODY_CAPTURES = createWeakSet();
/** Tests module-private provenance for the shared replay-body snapshot. */
export const isCapturedCanonicalReplayBody = (value) => value !== null &&
    typeof value === "object" &&
 ...
 */
const captureReplayBodyWithVisitor = (events, containingOperation, replayVisitor) => {
    if (!Array.isArray(events)) {
        throw new TypeError("Canonical replay events must be a dense bounded list.");
    }
    const replayBody = Object.create(null);
    Object.defineDataProperty(replayBody, "operationVersion", REPLAY_OPERATION_VERSION, false, true);
    Object.defineDataProperty(replayBody, "events", events, false, true);
    Object.freeze(replayBody);
    const captured = deepCapture(replayBody, "STRUCTURAL_PHASE_ONE", MAX_CANONICAL_UTF8_BYTES, {
...
export const captureBoundedCanonicalReplayBody = (events, containingOperation) => captureReplayBodyWithVisitor(events, containingOperation);
/**
 ...
 */
export const captureBoundedCanonicalReplayBodyIncrementally = (events, containingOperation, visitor) => {
    if (typeof visitor !== "function") {
        throw new TypeError("Canonical replay capture visitor must be a function.");
    }
    try {
        const capture = captureReplayBodyWithVisitor(events, containingOperation, visitor);
        ...
        if (error !== null &&
            ...
            return Object.freeze({
                status: "STOPPED",
                result: weakMapGet(REPLAY_CAPTURE_VISITOR_STOPS, error),
            });
        }
        if (error !== null &&
            typeof error === "object" &&
...
const CANONICAL_AUTHORITY_OPERATION_CAPTURES = createWeakSet();
/** Tests package-internal provenance for one complete authority-operation snapshot. */
export const isCapturedCanonicalAuthorityOperation = (value) => value !== null &&
    typeof value === "object" &&
 ...
 * `replayPortable`.
 */
export const captureBoundedCanonicalAuthorityOperationIncrementally = (value, containingOperation, kind, replayVisitor) => {
    try {
 ...
 */
export const captureBoundedCanonicalReceiptOperation = (input, kind, validateMember, preflightHistoryLimits) => {
    let artifactPhase = false;
    const invalid = (limitExceeded = false) => Object.freeze({ status: "INVALID_INPUT", limitExceeded });
    try {
        ...
        if (rawKeys.length !== expectedKeys.length)
            return invalid();
        const membership = createSet();
        ...
        for (let index = 0; index < expectedKeys.length; index += 1) {
            if (!setHas(membership, expectedKeys[index]))
                return invalid();
        }
        const context = {
            ...
            localBudgets: [],
            replayFailureToken: Object.freeze({}),
            preobservedPrototypes: createWeakMap(),
            replayVisitor: undefined,
        ...
        const capturedRecordKeys = Object.freeze((candidate) => candidate !== null && typeof candidate === "object"
            ? weakMapGet(context.recordKeysByCapturedValue, candidate)
            : undefined);
        const capturedCanonicalBytes = Object.freeze((candidate) => candidate !== null && typeof candidate === "object"
            ? weakMapGet(context.nodeByCapturedValue, candidate)?.canonicalBytes
            : undefined);
        const metadata = Object.freeze({ capturedRecordKeys });
        const value = Object.create(null);
        let maximumChildDepth = 0;
        const captureMember = (key, role = NORMAL_CAPTURE_ROLE) => {
            const location = `$.${key}`;
        ...
        if (typeof operationVersion !== "string")
            return invalid();
        ...
        const validate = (key, memberKind) => validateMember(metadata, memberKind, captureMember(key));
        const domainResult = validate("expectedDomain", "DOMAIN");
        if (domainResult !== "VALID")
            return invalid(domainResult === "LIMIT");
        if (kind === "VERIFY") {
            const timeResult = validate("verifierTime", "TIME");
            if (timeResult !== "VALID")
                return invalid(timeResult === "LIMIT");
            ...
            if (headResult !== "VALID")
                return invalid(headResult === "LIMIT");
            const idResult = validate("recordEventId", "IDENTIFIER");
            if (idResult !== "VALID")
                return invalid(idResult === "LIMIT");
            captureMember("events", REPLAY_EVENTS_CAPTURE_ROLE);
        }
        const histories = kind === "VERIFY"
            ? Object.freeze([value.issuanceEvents, value.observedEvents])
            : Object.freeze([value.events]);
        if (preflightHistoryLimits(Object.freeze({ capturedRecordKeys, capturedCanonicalBytes }), histories) === "LIMIT")
            return invalid(true);
        artifactPhase = true;
        ...
        const restored = createWeakMap();
        const restorePresence = (candidate) => {
            ...
            if (Array.isArray(candidate)) {
                ...
                for (let index = 0; index < keys.length; index += 1) {
                    const key = keys[index];
                    Object.defineDataProperty(copy, key, restorePresence(candidate[key]), false, true);
                }
        ...
        const restoredValue = restorePresence(value);
        const capture = Object.freeze({
            value: restoredValue,
 ...
 */
export const captureBoundedCanonicalQueryOperation = (input, kind) => {
    const invalid = () => Object.freeze({ status: "INVALID_INPUT" });
    try {
        if (input === null || typeof input !== "object" || Array.isArray(input))
            return invalid();
        const prototype = readPrototype(input, "$");
        ...
        const context = {
            mode: "STRICT_CANONICAL",
            budget: { label: "$", limit: MAX_CANONICAL_UTF8_BYTES, used: 0 },
            active: createWeakSet(), memo: createWeakMap(),
            recordKeysByCapturedValue: createWeakMap(),
            normalizedObjectMembers: createWeakMap(),
            nodeByCapturedValue: createWeakMap(), localBudgets: [],
            replayFailureToken: Object.freeze({}),
            preobservedPrototypes: createWeakMap(),
        ...
        for (let index = 0; index < required.length; index += 1) {
            if (!setHas(membership, required[index]))
                return invalid();
        }
        ...
        for (let index = 0; index < order.length; index += 1) {
            ...
            if (!setHas(membership, key))
                continue;
            const role = key === "evaluationEvents" || key === "observedEvents"
...
const CANONICAL_EVENT_DATA_SHAPE_ERRORS = createWeakSet();
const canonicalEventDataShapeError = () => {
...
export const isCanonicalEventDataShapeError = (error) => error !== null &&
    typeof error === "object" &&
 ...
 */
export const captureBoundedCanonicalEventData = (value, containingEvent, maxCanonicalBytes) => {
...
export const canonicalEncode = (value) => captureBoundedCanonicalValue(value).canonicalText;
/**
 ...
 */
export const immutableProtocolValue = (value) => deepCapture(value, "STRUCTURAL_PHASE_ONE", MAX_CANONICAL_UTF8_BYTES).value;
/**
 ...
 */
export const immutableProtocolInput = (value) => {
    const captured = deepCapture(value, "STRUCTURAL_PHASE_ONE", MAX_CANONICAL_UTF8_BYTES);
    const copies = createWeakMap();
    const restore = (node) => {
        ...
        if (Array.isArray(node)) {
            ...
            for (let index = 0; index < node.length; index += 1)
                arrayPush(copy, restore(node[index]));
            return Object.freeze(copy);
        }
        const keys = captured.capturedRecordKeys(node);
        if (keys === undefined)
            throw new TypeError("Input snapshot is missing record-key metadata.");
        const copy = Object.create(null);
        weakMapSet(copies, node, copy);
        for (let index = 0; index < keys.length; index += 1) {
            const key = keys[index];
            Object.defineDataProperty(copy, key, restore(node[key]), false, true);
        }
        return Object.freeze(copy);
    };
...
export const isDenseArray = (value, maximumLength = MAX_CANONICAL_LIST_MEMBERS) => {
    if (!Array.isArray(value))
        return false;
 ...
 */
export const hashEventHistoryPrefixes = (events) => {
    if (!Array.isArray(events)) {
        throw new TypeError("Event history must be a runtime array.");
    }
    const wrapper = [];
    ...
    for (let position = 0; position < capturedEvents.length; position += 1) {
        ...
        const eventBytes = UTF8_ENCODER.encode(canonicalEncode(capturedEvents[position]));
