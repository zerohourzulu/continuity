...
The coordinated preview uses `@ramex-labs/continuity@0.3.0-preview.8` as the shared engine. Remote preview.3, evidence MCP preview.9 and gateway preview.8 depend on that exact Core version; they do not contain private engine copies. See [the current package table](SHARED-CORE.md). The previously published packages remain available and unchanged.
