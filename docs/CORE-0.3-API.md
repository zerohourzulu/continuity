...
`obligate` and `assign` use signed engine transitions and recognize exact completed commands on repetition. Managed writes now share the96-event reserved-capacity profile; inspection still accepts previously supported256-event histories. Use `owner.capacity()` and [the capacity guide](LOCAL-CAPACITY.md) before new work. Handover consists of two visible durable events, not an atomic multi-event transaction. If writing fails after retirement, that retirement remains in force. Inspect the original file and rerun the same handover command to finish its missing transfer. Do not restore a pre-retirement snapshot.
...
The coordinated preview uses `@ramex-labs/continuity@0.3.0-preview.10` as the shared engine. Remote preview.3, evidence MCP preview.9 and gateway preview.8 depend on that exact Core version; they do not contain private engine copies. See [the current package table](SHARED-CORE.md). The previously published packages remain available and unchanged.
...
For an ordinary application, start with `/local`, `/runtime`, `/evidence` and `/attempts`; choose the remote integration only when the destination supports its contract. See [the shared-package guide](SHARED-CORE.md).
