...
Use the coordinated versions in [supported profiles](SUPPORTED-PROFILES.md). Install the bundled archives with `node tools/release/install.mjs /absolute/empty/consumer-directory`. The commands below then use that directory's `node_modules/.bin/continuity-history-store`. Keep all paths, keys and maintenance commands under operator control; never expose them as agent tools.
...
Run the [installed integration checks](CONTINUATION.md#reproduce-the-evaluation) on a disposable case before operating your own. These procedures assume cooperative local writers and trusted storage; they do not provide automatic disaster recovery, key recovery or protection from a hostile host.
