...
Need several approved tools rather than this single evidence example? Start with the [gateway guide](https://github.com/zerohourzulu/continuity/blob/main/docs/MCP-GATEWAY.md).
...
npm init -y
npm install --ignore-scripts @ramex-labs/continuity-mcp@0.3.0-preview.11
...
The package also supports `npx @ramex-labs/continuity-mcp@0.3.0-preview.11 --config /absolute/path/gateway.json`. The configuration must be created first. This launches the same protected server as `continuity-evidence`.

## Shared Core

This MCP preview depends on exactly `@ramex-labs/continuity@0.3.0-preview.10`; npm installs it automatically. The MCP archive contains no engine copy. Core owns the authority and history rules; this package connects them to the bounded evidence tool. The protocol, tool and Registry name remain unchanged.
