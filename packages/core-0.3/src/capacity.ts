...
const dutyExtension = new Set(["ATTEMPT_DUTY_POLICY_ACTIVATED", "ATTEMPT_DUTY_DISPOSITION_RECORDED", "ATTEMPT_DUTY_CONTEST_RECORDED"]);
...
export function dutyReservationOf(events: readonly PortableCanonicalEvent[]) {
  const duties = new Map<string, { dispositions: number; contests: number }>();
  let withinLimits = true;
  for (const event of events) {
    const d = event.data as unknown as Record<string, any>;
    if (event.type === "ATTEMPT_DUTY_POLICY_ACTIVATED") {
      // Selection and activation uniqueness belong to replay. Fixed D1 quotas
      // never come from caller-supplied descriptor limits.
      if (!duties.has(d.descriptor.dutyId)) duties.set(d.descriptor.dutyId, { dispositions: 0, contests: 0 });
    } else if (event.type === "ATTEMPT_DUTY_DISPOSITION_RECORDED" || event.type === "ATTEMPT_DUTY_CONTEST_RECORDED") {
      const duty = duties.get(d.dutyId);
      if (!duty) { withinLimits = false; continue; }
      if (event.type === "ATTEMPT_DUTY_DISPOSITION_RECORDED") duty.dispositions++;
      else duty.contests++;
      if (duty.dispositions > 4 || duty.contests > 2) withinLimits = false;
    }
  }
  // An invalid over-quota candidate must never manufacture negative reserves.
  const dutyReserved = [...duties.values()].reduce((sum, duty) =>
    sum + Math.max(0, 4 - duty.dispositions) + Math.max(0, 2 - duty.contests), 0);
  return Object.freeze({ activatedDuties: duties.size, dutyReserved, withinLimits });
}
...
export function capacityOf(events: readonly PortableCanonicalEvent[]) {
  ...
  const { activatedDuties, dutyReserved } = dutyReservationOf(events);
  let started = false, controlsUsed = 0, shape = true, extended = false;
  for (const event of events) {
    if (!bounded(event)) shape = false;
    const d = event.data as unknown as Record<string, any>;
    if (dutyExtension.has(event.type)) extended = true;
  ...
  const reserved = lifecycleReserved + controlReserved + dutyReserved;
  const free = LOCAL_CAPACITY_PROFILE.maxEvents - events.length - reserved;
  // D1 writers require the segmented store, even when a history fits managed96.
  const compatible = shape && !extended && free >= 0;
  return Object.freeze({ profile: LOCAL_CAPACITY_PROFILE.version, eventCount: events.length,
    ...
    ...(activatedDuties ? { activatedDuties, dutyReserved } : {}),
...
export function assertCapacityTransition(events: readonly PortableCanonicalEvent[], next: readonly PortableCanonicalEvent[]) {
  ...
  if (next.some(event=>dutyExtension.has(event.type))) error("CAPACITY_INCOMPATIBLE");
