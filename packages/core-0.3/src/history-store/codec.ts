...
import { CONTINUATION_PROFILE as P } from '../history.ts';
export class HistoryStoreError extends Error {
    readonly code: string;
    constructor(code: string) { super(code); this.name = 'HistoryStoreError'; this.code = code; }
}
export function fail(code: string): never { throw new HistoryStoreError(code); }
...
export function exact(value: unknown, keys: readonly string[]): asserts value is Record<string, any> {
    if (!value || typeof value !== 'object' || Array.isArray(value) ||
        (Object.keys(value).length !== keys.length || Object.keys(value).some(k => !keys.includes(k))))
        fail('FORMAT_INVALID');
}
export const integer = (v: unknown, max: number) => typeof v === 'number' && Number.isSafeInteger(v) && v >= 0 && !Object.is(v, -0) && v <= max;
export const digest = (v: unknown) => typeof v === 'string' && /^[0-9a-f]{64}$/.test(v);
export const uuid = (v: unknown) => typeof v === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(v);
export const json = (v: unknown) => Buffer.from(JSON.stringify(v) + '\n');
export function parse(bytes: Buffer) {
    try {
...
const encode = (v: any): any => v === null ? ['null'] : Array.isArray(v) ? ['array', v.map(encode)] :
...
export function decodeEvent(bytes: Buffer): unknown {
    if (bytes.length > LIMITS.eventBytes)
        fail('ENCODED_EVENT_LIMIT');
