...
export type AttemptDutyAssignment = Readonly<{ id: string; duty: string }>;
export type AttemptDutyReview = Readonly<{ id: string; duty: string; summaryDigest: core.ContentHash }>;
type AttemptEvent = "OUTCOME_OBSERVATION_RECORDED" | "ATTEMPT_DUTY_CREATED" | "ATTEMPT_DUTY_ASSIGNED" | "ATTEMPT_DUTY_REVIEW_CLOSED";
const attemptPolicy = (hash: core.ContentHash) => hash === core.PORTABLE_ADAPTER_POLICY_E5_HASH || hash === core.PORTABLE_ADAPTER_POLICY_E6_HASH;

function selectedSource(state: ReturnType<typeof stateOf>, intentId: string) {
...
export function openLocalAttemptRecorder(options: LocalRuntimeOptions) {
  ...
  const signed = async (
    ...
    if (existing) {
      const { administrativeAuthorization: _authorization, ...effect } = existing.data as Record<string, unknown>;
    ...
    const produced = await produceAdministrativeEvent(input, {
      signHash: async (hash: core.ContentHash) => {
        try { return await signer(hash); }
        catch { throw new ContinuityError("SIGNER_FAILED"); }
      },
    });
  ...
  return Object.freeze({
    async observe(input: OutcomeObservation): Promise<WriteResult> {
      ...
      const acknowledgment = value.acknowledgment as core.PortableAdapterAcknowledgment;
    ...
    async createDuty(input: AttemptDuty): Promise<WriteResult> {
      ...
      if (existing) {
        ...
        const { administrativeAuthorization: _authorization, ...effect } = existing.data as Record<string, unknown>;
        const original = effect.record as core.PortableAttemptDutyRecord;
      ...
      const duty: core.PortableAttemptDutyRecord = {
    ...
    async reviewDuty(input: AttemptDutyReview): Promise<WriteResult> {
    ...
    async assignDuty(input: AttemptDutyAssignment): Promise<WriteResult> {
...
function inspectAttempts(events: readonly core.PortableCanonicalEvent[]) {
  ...
  return core.immutableProtocolValue({
    scope: "CAPTURED_HISTORY_ONLY" as const, executionCapability: false as const,
