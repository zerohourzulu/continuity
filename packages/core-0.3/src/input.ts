...
export class ContinuityError extends Error {
  ...
  constructor(code: ContinuityErrorCode, mayHaveCommitted = false) {
    super(code);
...
export function requireCondition(
  value: unknown,
  code: ContinuityErrorCode = "INVALID_INPUT",
): asserts value {
  if (!value) throw new ContinuityError(code);
}
...
export function captureData(
  ...
  const copy = (value: unknown, depth: number): unknown => {
    ...
    requireCondition(
      keys.length <= maxNodes && keys.every((key) => typeof key === "string"),
    );
    ancestors.add(value);
    try {
      if (array) {
...
export function identifiers(value: unknown): readonly string[] {
  requireCondition(
