...
export function inspectCase({historyFile,storage}){
 const directory=privateDirectory(storage);fileBytes(historyFile,8388608);
 const state=stateOf(new PortableFileEventStore(historyFile).readAll()),names=recordNames(directory);
 let recordBytes=0;
 ...
 return {profile:'continuity-local-evaluation/1',eventCount:count,limits:{coreEvents:256,observationEvents:128,recordEntries:4096},
  remainingCoreEvents:Math.max(0,256-count),lateObservationCapacityAvailable:count<128,
  ...
  leasePresent:existsSync(join(directory,LOCK)),warnings:[...(count>=128?['LATE_OBSERVATION_LIMIT_REACHED']:[]),...(count>=200?['CORE_CAPACITY_LOW']:[])],
...
export function snapshotCase({historyFile,storage,directory}){
