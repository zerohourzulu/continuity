import {
  ...
  REMOTE_SERVICE_REPORT_ACKNOWLEDGMENT_VERSION,
} from "./portable-adapter-engine.ts";
import {
  compareProtocolStrings,
  type CapturedCanonicalReplayEvent,
  type CapturedCanonicalReplayBody,
  captureBoundedCanonicalEventData,
  isCapturedCanonicalReplayEvent,
  isCapturedCanonicalReplayBody,
  isCanonicalCaptureLimitError,
  isCanonicalEventDataShapeError,
} from "./canonical.ts";
import {
  arrayIncludes,
  arrayIsArray,
...
const EVENT_TYPE_RANK = createMap<CoreEventType, number>();
for (let rank = 0; rank < CORE_EVENT_TYPES.length; rank += 1) {
  mapSet(EVENT_TYPE_RANK, CORE_EVENT_TYPES[rank]!, rank);
}

export const isCoreEventType = (value: unknown): value is CoreEventType =>
  typeof value === "string" && mapHas(EVENT_TYPE_RANK, value as CoreEventType);

/** The exact zero-based Section 5.1 rank, or undefined for an unknown type. */
export const coreEventTypeRank = (value: unknown): number | undefined =>
  isCoreEventType(value) ? mapGet(EVENT_TYPE_RANK, value) : undefined;
...
export interface AcceptedGenesisVersionProbe
  extends AcceptedCanonicalBaseEventShape {
...
export type EventShapeFailureCode =
...
const CORE_SCHEMA_LIMIT_FAILURES = createWeakSet<object>();

/** @internal Authentic provenance for one snapshot-only named hard-limit failure. */
export const isCoreSchemaLimitFailure = (value: unknown): boolean =>
  value !== null &&
  typeof value === "object" &&
  weakSetHas(CORE_SCHEMA_LIMIT_FAILURES, value);

const brandSchemaLimitFailure = <T extends object>(value: T): T => {
...
class ShapeValidator {
  ...
  failureKind: "SEMANTIC" | "LIMIT" | undefined;
  private readonly capturedRecordKeys!: (
    value: unknown,
  ...
  failLimit(path: string, message: string): false {
  ...
  record(
    ...
    const allowed = createSet<string>();
...
type BoundedUtf8Status = "VALID" | "MALFORMED" | "LIMIT";

const boundedWellFormedUtf8Status = (
  value: string,
  maximumBytes: number,
): BoundedUtf8Status => {
...
const identifier = (v: ShapeValidator, value: unknown, path: string): value is string => {
...
const protocolString = (
  ...
  if (utf8Status === "MALFORMED") return v.fail(path, "must be a bounded ProtocolString");
  return true;
};

const u53 = (v: ShapeValidator, value: unknown, path: string): value is number => {
  if (
    typeof value !== "number" ||
...
const amount = (v: ShapeValidator, value: unknown, path: string): value is bigint => {
  ...
  if (value < 0n) return v.fail(path, "must be an Amount");
  return true;
};

const contentHash = (v: ShapeValidator, value: unknown, path: string): value is string => {
...
const chainId = (v: ShapeValidator, value: unknown, path: string): value is string => {
...
const signature65 = (v: ShapeValidator, value: unknown, path: string): value is string => {
...
const literal = (
  ...
  typeof value === "string" && arrayIncludes(allowed, value)
    ? true
...
const identifierList = (
  ...
  value: unknown,
  path: string,
  options: { readonly nonempty?: boolean; readonly maximum?: number } = {},
): value is readonly string[] => {
  if (!Array.isArray(value)) return v.fail(path, "must be a dense Identifier list");
  ...
  for (let index = 0; index < value.length; index += 1) {
    if (!identifier(v, value[index], `${path}[${index}]`)) return false;
    if (setHas(seen, value[index]!)) return v.fail(`${path}[${index}]`, "duplicates an Identifier");
    setAdd(seen, value[index]!);
  }
  return true;
};

const reviewObservationIds = (v: ShapeValidator, value: unknown, path: string): boolean => {
  if (!identifierList(v, value, path, { maximum: 128 })) return false;
  for (let index = 1; index < value.length; index += 1) {
    if (compareProtocolStrings(value[index - 1]!, value[index]!) >= 0) return v.fail(path, "must be strictly sorted");
  }
  return true;
};
...
const enumList = (
  ...
  const seen = createSet<string>();
  for (let index = 0; index < value.length; index += 1) {
    if (!literal(v, value[index], `${path}[${index}]`, allowed)) return false;
    if (setHas(seen, value[index]!)) return v.fail(`${path}[${index}]`, "duplicates a member");
    setAdd(seen, value[index]!);
  }
  return true;
};

const authorizationDomain = (v: ShapeValidator, value: unknown, path: string): boolean => {
  const record = v.record(
    value,
    path,
    ["protocol", "version", "deploymentId", "chainId", "verifyingContract"],
  );
...
const versionSet = (v: ShapeValidator, value: unknown, path: string): boolean => {
...
const declaredVersionSet = (
  v: ShapeValidator,
  value: unknown,
  path: string,
): value is DeclaredVersionSet => {
  const record = v.record(value, path, [
    "eventSchemaVersion",
    "receiptSchemaVersion",
    "queryEnvelopeVersion",
  ...
  return record !== undefined &&
    ...
    identifier(
      v,
      record.administrativeAuthorizationVersion,
      `${path}.administrativeAuthorizationVersion`,
    ) &&
    identifier(v, record.signatureScheme, `${path}.signatureScheme`);
};

const historyHead = (v: ShapeValidator, value: unknown, path: string): boolean => {
...
const actionRequest = (v: ShapeValidator, value: unknown, path: string): boolean => {
  const record = v.record(
    value,
    path,
    ["actorId", "action", "resource", "claimedAt"],
    ["amount", "counterpartyId", "termsCommitment"],
  );
  return record !== undefined &&
...
const evidenceReference = (v: ShapeValidator, value: unknown, path: string): boolean => {
...
const authorityConstraints = (
  ...
  value: unknown,
  path: string,
  requiredIntersectionMaximum = MAX_REQUIRED_INTERSECTIONS,
): boolean => {
  const record = v.record(
    value,
    path,
    ["actions", "resources", "quantitative", "maxDelegationDepth", "requiredIntersectionIds"],
    ["notBefore", "expiresAt", "maxAmount", "maxCumulativeAmount", "maxTransactions"],
  );
  if (record === undefined) return false;
  ...
  if (
    Array.isArray(record.requiredIntersectionIds) &&
    record.requiredIntersectionIds.length > requiredIntersectionMaximum
  ) {
    return v.failLimit(
      `${path}.requiredIntersectionIds`,
      `exceeds the ${requiredIntersectionMaximum}-member required-intersection bound`,
    );
  }
  if (
    !identifierList(v, record.actions, `${path}.actions`, {
      nonempty: true,
...
const permissionGrant = (v: ShapeValidator, value: unknown, path: string): boolean => {
  const record = v.record(
    value,
    path,
    [
...
const prohibitionGrant = (v: ShapeValidator, value: unknown, path: string): boolean => {
  ...
  if (value.scope === "ROOT") {
    const record = v.record(
      value,
      path,
      ["kind", "scope", "authorityId", "grantorId", "rootAuthorityId", "constraints"],
      ["subjectActorId", "parentAuthorityId"],
    );
    return record !== undefined &&
      identifier(v, record.authorityId, `${path}.authorityId`) &&
      identifier(v, record.grantorId, `${path}.grantorId`) &&
  ...
  if (value.scope === "GLOBAL") {
    const record = v.record(
      value,
      path,
      ["kind", "scope", "authorityId", "grantorId", "constraints"],
      ["subjectActorId"],
    );
    return record !== undefined &&
      identifier(v, record.authorityId, `${path}.authorityId`) &&
...
const authorityGrant = (v: ShapeValidator, value: unknown, path: string): boolean => {
  if (!isPlainRecord(value)) return v.fail(path, "must be an authority grant record");
  if (value.kind === "PERMISSION") return permissionGrant(v, value, path);
  if (value.kind === "PROHIBITION") return prohibitionGrant(v, value, path);
  return v.fail(`${path}.kind`, "must be PERMISSION or PROHIBITION");
};

const recognizedRoot = (v: ShapeValidator, value: unknown, path: string): boolean => {
  const record = v.record(value, path, [
    "rootAuthorityId",
    "principalId",
...
const authorityUsage = (v: ShapeValidator, value: unknown, path: string): boolean => {
  const record = v.record(value, path, [
    "authorityId",
...
const authorityEvidence = (v: ShapeValidator, value: unknown, path: string): boolean => {
...
const permissionGrantList = (
  ...
  if (nonempty && value.length === 0) return v.fail(path, "must be a bounded permission path");
  const seen = createSet<string>();
  for (let index = 0; index < value.length; index += 1) {
    if (!permissionGrant(v, value[index], `${path}[${index}]`)) return false;
    const authorityId = (value[index] as Record<string, unknown>).authorityId as string;
    if (setHas(seen, authorityId)) return v.fail(`${path}[${index}]`, "duplicates a path authority");
    setAdd(seen, authorityId);
  }
  return true;
};

const intersectionProof = (v: ShapeValidator, value: unknown, path: string): boolean => {
  const record = v.record(value, path, [
    "requiredAuthorityId",
  ...
  return record !== undefined &&
    ...
    authorityConstraints(
      v,
      record.effectiveConstraints,
      `${path}.effectiveConstraints`,
      MAX_SET_MEMBERS,
    );
};

const authorizationProof = (v: ShapeValidator, value: unknown, path: string): boolean => {
  ...
  if (Array.isArray(record.intersections) && record.intersections.length > MAX_SET_MEMBERS) {
    return v.failLimit(
      `${path}.intersections`,
      `exceeds the ${MAX_SET_MEMBERS}-member proof-intersection bound`,
    );
  }
  if (Array.isArray(record.usageSnapshot) && record.usageSnapshot.length > MAX_SET_MEMBERS) {
    return v.failLimit(
      `${path}.usageSnapshot`,
      `exceeds the ${MAX_SET_MEMBERS}-member authority-usage bound`,
    );
  }
  if (Array.isArray(record.authorityEvidence) && record.authorityEvidence.length > MAX_AUTHORITIES) {
    return v.failLimit(
      `${path}.authorityEvidence`,
      `exceeds the ${MAX_AUTHORITIES}-member authority-evidence bound`,
    );
  }
  if (
    record.proofVersion !== "continuity-authorization-proof/0.2" ||
    ...
    if (v.problem === undefined) v.fail(path, "contains an invalid AuthorizationProof field");
    return false;
  }

  const intersectionIds = createSet<string>();
  for (let index = 0; index < record.intersections.length; index += 1) {
    const member = record.intersections[index];
    if (!intersectionProof(v, member, `${path}.intersections[${index}]`)) return false;
    const id = (member as Record<string, unknown>).requiredAuthorityId as string;
  ...
  const usageIds = createSet<string>();
  for (let index = 0; index < record.usageSnapshot.length; index += 1) {
    const member = record.usageSnapshot[index];
    if (!authorityUsage(v, member, `${path}.usageSnapshot[${index}]`)) return false;
    const id = (member as Record<string, unknown>).authorityId as string;
    if (setHas(usageIds, id)) return v.fail(`${path}.usageSnapshot[${index}]`, "duplicates an authority");
    setAdd(usageIds, id);
  }
  ...
  for (let index = 0; index < record.authorityEvidence.length; index += 1) {
    ...
    if (!authorityEvidence(v, member, `${path}.authorityEvidence[${index}]`)) return false;
    const id = (member as Record<string, unknown>).authorityId as string;
...
const adapterProfile = (v: ShapeValidator, value: unknown, path: string): boolean => {
...
const adapterEvidence = (v: ShapeValidator, value: unknown, path: string, noEffect = false): boolean => {
  ...
  if (record === undefined || !adapterProfile(v, record.adapterProfile, `${path}.adapterProfile`) ||
      !authorizationDomain(v, record.domain, `${path}.domain`) ||
      !historyHead(v, record.admissionHead, `${path}.admissionHead`)) return false;
  const result = record.result;
  if (!isPlainRecord(result)) return false;
  if (noEffect) {
    if (v.record(result, `${path}.result`, ["kind", "reference"]) === undefined) return false;
    return validatePortableAdapterNoEffectShape(record);
  }
  if (result.kind === "SIMULATED_SUBMISSION") {
    if (v.record(result, `${path}.result`, ["kind", "submissionReference"]) === undefined) return false;
  } else if (result.kind === "LOCAL_PACKET_CREATED") {
    if (v.record(result, `${path}.result`, ["kind", "manifestDigest"]) === undefined ||
        v.record(result.manifestDigest, `${path}.result.manifestDigest`, ["algorithm", "value"]) === undefined) return false;
  } else if (result.kind === "REMOTE_SERVICE_REPORTED") {
    if (v.record(result, `${path}.result`, ["kind", "reportDigest"]) === undefined ||
        v.record(result.reportDigest, `${path}.result.reportDigest`, ["algorithm", "value"]) === undefined) return false;
  } else if (result.kind === "LOCAL_DOCUMENT_RELEASED") {
    if (v.record(result, `${path}.result`, ["kind", "publicationManifestDigest"]) === undefined ||
        v.record(result.publicationManifestDigest, `${path}.result.publicationManifestDigest`, ["algorithm", "value"]) === undefined) return false;
  } else if (result.kind === "LOCAL_ENDPOINT_STATE_CHANGED") {
    if (v.record(result, `${path}.result`, ["kind", "transitionDigest"]) === undefined ||
        v.record(result.transitionDigest, `${path}.result.transitionDigest`, ["algorithm", "value"]) === undefined) return false;
  } else return false;
  return validatePortableAdapterAcknowledgmentShape(record);
};

const transactionIntent = (v: ShapeValidator, value: unknown, path: string): boolean => {
  const record = v.record(
    value,
    path,
    ["intentId", "nonce", "actorId", "action", "resource", "roleId", "roleTenureId", "adapterProfile"],
    ["amount", "counterpartyId", "termsCommitment"],
  );
  return record !== undefined &&
...
const obligationTransitionPolicy = (
  v: ShapeValidator,
  value: unknown,
  path: string,
): boolean => {
...
const obligationRecord = (v: ShapeValidator, value: unknown, path: string): boolean => {
  const record = v.record(
    value,
    path,
    [
  ...
  if (
    ...
    if (v.problem === undefined) v.fail(path, "contains an invalid ObligationRecord field");
    return false;
  }
  const pairs = createSet<string>();
  for (let index = 0; index < record.transitionPolicies.length; index += 1) {
    ...
    const policy = member as Record<string, unknown>;
    const pair = `${policy.fromStatus as string}\u0000${policy.toStatus as string}`;
...
const obligationCreatedData = (v: ShapeValidator, value: unknown, path: string): boolean => {
...
type NamedLimitNode =
  | Readonly<{ readonly kind: "IDENTIFIER" }>
  | Readonly<{ readonly kind: "BIGINT" }>
  | Readonly<{
      readonly kind: "RECORD";
      readonly fields: readonly NamedLimitField[];
    }>
  | Readonly<{
      readonly kind: "LIST";
      readonly maximum?: number;
      readonly boundName?: string;
      readonly member?: NamedLimitNode;
    }>
  | Readonly<{
      readonly kind: "TAGGED_RECORD";
      readonly tagField: string;
      readonly common?: NamedLimitNode;
      readonly variants: readonly NamedLimitVariant[];
    }>;

type NamedLimitField = readonly [name: string, node: NamedLimitNode];
type NamedLimitVariant = readonly [tag: string, node: NamedLimitNode];

const LIMIT_IDENTIFIER = Object.freeze({ kind: "IDENTIFIER" } as const);
const LIMIT_BIGINT = Object.freeze({ kind: "BIGINT" } as const);

const limitField = (name: string, node: NamedLimitNode): NamedLimitField =>
  Object.freeze([name, node] as const);

const limitVariant = (tag: string, node: NamedLimitNode): NamedLimitVariant =>
  Object.freeze([tag, node] as const);

const limitRecord = (...fields: readonly NamedLimitField[]): NamedLimitNode =>
  Object.freeze({ kind: "RECORD", fields: Object.freeze(fields) });

const limitList = (
  member?: NamedLimitNode,
  maximum?: number,
  boundName?: string,
): NamedLimitNode => Object.freeze({
  kind: "LIST",
  member,
  maximum,
  boundName,
...
const limitTaggedRecord = (
  tagField: string,
  variants: readonly NamedLimitVariant[],
  common?: NamedLimitNode,
): NamedLimitNode => Object.freeze({
  kind: "TAGGED_RECORD",
  tagField,
...
const LIMIT_DOMAIN = limitRecord(
  limitField("deploymentId", LIMIT_IDENTIFIER),
);

const LIMIT_ADAPTER_PROFILE = limitRecord(limitField("profileId", LIMIT_IDENTIFIER), limitField("profileVersion", LIMIT_IDENTIFIER));
const LIMIT_ADAPTER_EVIDENCE = limitRecord(
  limitField("schemaVersion", LIMIT_IDENTIFIER), limitField("kind", LIMIT_IDENTIFIER),
  limitField("adapterProfile", LIMIT_ADAPTER_PROFILE), limitField("domain", LIMIT_DOMAIN),
  limitField("intentId", LIMIT_IDENTIFIER),
  limitField("result", limitRecord(limitField("kind", LIMIT_IDENTIFIER),
    limitField("reportDigest", limitRecord(limitField("algorithm", LIMIT_IDENTIFIER))),
    limitField("publicationManifestDigest", limitRecord(limitField("algorithm", LIMIT_IDENTIFIER))),
    limitField("manifestDigest", limitRecord(limitField("algorithm", LIMIT_IDENTIFIER))),
    limitField("transitionDigest", limitRecord(limitField("algorithm", LIMIT_IDENTIFIER))))),
);

const LIMIT_VERSION_SET = limitRecord(
  limitField("eventSchemaVersion", LIMIT_IDENTIFIER),
  limitField("receiptSchemaVersion", LIMIT_IDENTIFIER),
  limitField("queryEnvelopeVersion", LIMIT_IDENTIFIER),
  limitField("authorizationProofVersion", LIMIT_IDENTIFIER),
  limitField("runtimeAuthorizationVersion", LIMIT_IDENTIFIER),
  limitField("administrativeAuthorizationVersion", LIMIT_IDENTIFIER),
  limitField("signatureScheme", LIMIT_IDENTIFIER),
);

const LIMIT_ACTION_REQUEST = limitRecord(
  limitField("actorId", LIMIT_IDENTIFIER),
  limitField("action", LIMIT_IDENTIFIER),
  limitField("resource", LIMIT_IDENTIFIER),
  limitField("amount", LIMIT_BIGINT),
  limitField("counterpartyId", LIMIT_IDENTIFIER),
);
...
const LIMIT_EVIDENCE_REFERENCE = limitTaggedRecord("kind", [
  ...
  limitVariant(
    "AUTHORITY",
    limitRecord(
      limitField("authorityId", LIMIT_IDENTIFIER),
      limitField("grantEventId", LIMIT_IDENTIFIER),
    ),
  ),
  limitVariant(
    "RUNTIME_CREDENTIAL",
    limitRecord(
      limitField("keyId", LIMIT_IDENTIFIER),
      limitField("sessionId", LIMIT_IDENTIFIER),
      limitField("admissionEventId", LIMIT_IDENTIFIER),
    ),
  ),
  limitVariant(
    "RECEIPT_COMMITMENT",
    limitRecord(limitField("eventId", LIMIT_IDENTIFIER)),
  ),
  limitVariant(
    "EXTERNAL",
    limitRecord(
      limitField("evidenceType", LIMIT_IDENTIFIER),
      limitField("reference", LIMIT_IDENTIFIER),
      limitField("attesterId", LIMIT_IDENTIFIER),
    ),
  ),
]);

const limitIdentifierSet = (boundName: string): NamedLimitNode =>
  limitList(LIMIT_IDENTIFIER, MAX_SET_MEMBERS, boundName);

const limitAuthorityConstraints = (
  requiredIntersectionMaximum: number,
): NamedLimitNode => limitRecord(
  limitField("actions", limitIdentifierSet("action set")),
  limitField("resources", limitIdentifierSet("resource set")),
  limitField(
    "requiredIntersectionIds",
    limitList(
      LIMIT_IDENTIFIER,
      requiredIntersectionMaximum,
      "required-intersection set",
    ),
  ),
  limitField("maxAmount", LIMIT_BIGINT),
  limitField("maxCumulativeAmount", LIMIT_BIGINT),
);

const LIMIT_GRANT_CONSTRAINTS = limitAuthorityConstraints(
  MAX_REQUIRED_INTERSECTIONS,
);
const LIMIT_EFFECTIVE_CONSTRAINTS = limitAuthorityConstraints(MAX_SET_MEMBERS);

const LIMIT_PERMISSION_GRANT = limitRecord(
  limitField("authorityId", LIMIT_IDENTIFIER),
  limitField("grantorId", LIMIT_IDENTIFIER),
  limitField("granteeId", LIMIT_IDENTIFIER),
  limitField("rootAuthorityId", LIMIT_IDENTIFIER),
  limitField("parentAuthorityId", LIMIT_IDENTIFIER),
  limitField("constraints", LIMIT_GRANT_CONSTRAINTS),
);

const LIMIT_AUTHORITY_GRANT = limitTaggedRecord(
  "kind",
  [
    limitVariant(
      "PERMISSION",
...
const LIMIT_RECOGNIZED_ROOT = limitRecord(
  limitField("rootAuthorityId", LIMIT_IDENTIFIER),
  limitField("principalId", LIMIT_IDENTIFIER),
  limitField("principalRecognitionEventId", LIMIT_IDENTIFIER),
  limitField("rootGrantEventId", LIMIT_IDENTIFIER),
);

const LIMIT_AUTHORITY_USAGE = limitRecord(
  limitField("authorityId", LIMIT_IDENTIFIER),
  limitField("admittedCumulativeAmount", LIMIT_BIGINT),
);

const LIMIT_AUTHORITY_EVIDENCE = limitRecord(
  limitField("authorityId", LIMIT_IDENTIFIER),
  limitField("grantEventId", LIMIT_IDENTIFIER),
);

const LIMIT_PERMISSION_PATH = limitList(
  LIMIT_PERMISSION_GRANT,
  MAX_AUTHORITY_PATH_DEPTH,
  "authority path",
);

const LIMIT_INTERSECTION_PROOF = limitRecord(
  limitField("requiredAuthorityId", LIMIT_IDENTIFIER),
  limitField(
    "requiredByAuthorityIds",
    limitIdentifierSet("required-by authority set"),
  ),
  limitField("recognizedRoot", LIMIT_RECOGNIZED_ROOT),
  limitField("path", LIMIT_PERMISSION_PATH),
  limitField("effectiveConstraints", LIMIT_EFFECTIVE_CONSTRAINTS),
);

const LIMIT_AUTHORIZATION_PROOF = limitRecord(
  limitField("domain", LIMIT_DOMAIN),
  limitField("policyVersion", LIMIT_IDENTIFIER),
  limitField("request", LIMIT_ACTION_REQUEST),
  limitField("recognizedRoot", LIMIT_RECOGNIZED_ROOT),
  limitField("permissionPath", LIMIT_PERMISSION_PATH),
  limitField(
    "intersections",
    limitList(
      LIMIT_INTERSECTION_PROOF,
      MAX_SET_MEMBERS,
      "proof-intersection set",
    ),
  ),
  limitField("effectiveConstraints", LIMIT_EFFECTIVE_CONSTRAINTS),
  limitField(
    "controllingAuthorityIds",
    limitIdentifierSet("controlling-authority set"),
  ),
  limitField(
    "usageSnapshot",
    limitList(
      LIMIT_AUTHORITY_USAGE,
      MAX_SET_MEMBERS,
      "authority-usage set",
    ),
  ),
  ...
  limitField(
    "checkedProhibitionIds",
    limitIdentifierSet("checked-prohibition set"),
  ),
  limitField("runtimeSessionId", LIMIT_IDENTIFIER),
  limitField("credentialKeyId", LIMIT_IDENTIFIER),
  limitField("roleId", LIMIT_IDENTIFIER),
  limitField("roleTenureId", LIMIT_IDENTIFIER),
  limitField("intentId", LIMIT_IDENTIFIER),
  limitField("nonce", LIMIT_IDENTIFIER),
);

const LIMIT_TRANSACTION_INTENT = limitRecord(
  limitField("adapterProfile", LIMIT_ADAPTER_PROFILE),
  limitField("intentId", LIMIT_IDENTIFIER),
  limitField("nonce", LIMIT_IDENTIFIER),
  limitField("actorId", LIMIT_IDENTIFIER),
  limitField("action", LIMIT_IDENTIFIER),
...
const LIMIT_OBLIGATION_TRANSITION_POLICY = limitRecord(
  limitField(
    "acceptedAttesterIds",
    limitIdentifierSet("accepted-attester set"),
  ),
  limitField("action", LIMIT_IDENTIFIER),
  limitField(
    "requiredAuthorityIds",
    limitIdentifierSet("required-authority set"),
  ),
);

const LIMIT_OBLIGATION_RECORD = limitRecord(
  limitField("obligationId", LIMIT_IDENTIFIER),
  limitField("sourceIntentId", LIMIT_IDENTIFIER),
  limitField("durableRoleId", LIMIT_IDENTIFIER),
  limitField("creationRoleTenureId", LIMIT_IDENTIFIER),
  limitField("trigger", LIMIT_IDENTIFIER),
  limitField("beneficiaryId", LIMIT_IDENTIFIER),
  limitField("counterpartyId", LIMIT_IDENTIFIER),
  limitField("performanceAssigneeId", LIMIT_IDENTIFIER),
  limitField("successionRuleId", LIMIT_IDENTIFIER),
  limitField(
    "transitionPolicies",
    limitList(LIMIT_OBLIGATION_TRANSITION_POLICY),
  ),
);

const LIMIT_ADMINISTRATIVE_CHALLENGE = limitRecord(
  limitField("domain", LIMIT_DOMAIN),
  limitField("request", LIMIT_ACTION_REQUEST),
  limitField("policyVersion", LIMIT_IDENTIFIER),
  limitField("transitionEventId", LIMIT_IDENTIFIER),
  limitField("runtimeSessionId", LIMIT_IDENTIFIER),
  limitField("credentialKeyId", LIMIT_IDENTIFIER),
  limitField("roleId", LIMIT_IDENTIFIER),
  limitField("roleTenureId", LIMIT_IDENTIFIER),
...
const LIMIT_ADMINISTRATIVE_AUTHORIZATION = limitRecord(
  limitField("challenge", LIMIT_ADMINISTRATIVE_CHALLENGE),
  limitField("authorityProof", LIMIT_AUTHORIZATION_PROOF),
);

const LIMIT_OBLIGATION_CREATED_DATA = limitRecord(
  limitField("record", LIMIT_OBLIGATION_RECORD),
  limitField("actorId", LIMIT_IDENTIFIER),
  limitField("administrativeAuthorization", LIMIT_ADMINISTRATIVE_AUTHORIZATION),
);

const LIMIT_ATTEMPT_DUTY_RECORD = limitRecord(
  limitField("dutyId", LIMIT_IDENTIFIER),
  limitField("sourceIntentId", LIMIT_IDENTIFIER),
  limitField("sourceAdmissionEventId", LIMIT_IDENTIFIER),
  limitField("durableRoleId", LIMIT_IDENTIFIER),
  limitField("creationRoleTenureId", LIMIT_IDENTIFIER),
  limitField("performanceAssigneeId", LIMIT_IDENTIFIER),
);

const CORE_EVENT_NAMED_LIMITS = createMap<CoreEventType, NamedLimitNode>();

mapSet(
  CORE_EVENT_NAMED_LIMITS,
  "DEPLOYMENT_INITIALIZED",
  limitRecord(
    limitField("domain", LIMIT_DOMAIN),
    limitField("canonicalLineageId", LIMIT_IDENTIFIER),
    limitField("versions", LIMIT_VERSION_SET),
    limitField("policyVersion", LIMIT_IDENTIFIER),
    limitField("globalPolicySourceId", LIMIT_IDENTIFIER),
  ),
);
mapSet(
  CORE_EVENT_NAMED_LIMITS,
  "PRINCIPAL_CREATED",
  limitRecord(limitField("principalId", LIMIT_IDENTIFIER)),
);
mapSet(
  CORE_EVENT_NAMED_LIMITS,
  "AGENT_CREATED",
  limitRecord(
    limitField("agentId", LIMIT_IDENTIFIER),
    limitField("principalId", LIMIT_IDENTIFIER),
    limitField("controllerId", LIMIT_IDENTIFIER),
  ),
);
mapSet(
  CORE_EVENT_NAMED_LIMITS,
...
mapSet(
  ...
  "SUCCESSION_RULE_DECLARED",
  limitRecord(
    limitField("ruleId", LIMIT_IDENTIFIER),
    limitField("principalId", LIMIT_IDENTIFIER),
    limitField("predecessorAgentId", LIMIT_IDENTIFIER),
    limitField("successorAgentId", LIMIT_IDENTIFIER),
    limitField("roleId", LIMIT_IDENTIFIER),
  ),
);
mapSet(
...
mapSet(
  ...
  limitRecord(
    limitField("roleId", LIMIT_IDENTIFIER),
    limitField("fromAgentId", LIMIT_IDENTIFIER),
    limitField("fromRoleTenureId", LIMIT_IDENTIFIER),
    limitField("toAgentId", LIMIT_IDENTIFIER),
    limitField("toRoleTenureId", LIMIT_IDENTIFIER),
    limitField("principalId", LIMIT_IDENTIFIER),
    limitField("successionRuleId", LIMIT_IDENTIFIER),
  ),
);
...
mapSet(
  CORE_EVENT_NAMED_LIMITS,
  "AUTHORITY_GRANTED",
  limitRecord(limitField("grant", LIMIT_AUTHORITY_GRANT)),
);
mapSet(
  CORE_EVENT_NAMED_LIMITS,
  "AUTHORITY_REVOKED",
  limitRecord(
    limitField("authorityId", LIMIT_IDENTIFIER),
    limitField("revokerId", LIMIT_IDENTIFIER),
  ),
);
...
mapSet(
  ...
  limitRecord(
    limitField("intentId", LIMIT_IDENTIFIER),
    limitField("authorizationProof", LIMIT_AUTHORIZATION_PROOF),
  ),
);
mapSet(
  CORE_EVENT_NAMED_LIMITS,
  ...
  limitRecord(
    limitField("intentId", LIMIT_IDENTIFIER),
    limitField("adapterId", LIMIT_IDENTIFIER),
    limitField("transactionReference", LIMIT_IDENTIFIER),
    limitField("acknowledgment", LIMIT_ADAPTER_EVIDENCE),
    limitField("evidenceReference", LIMIT_EVIDENCE_REFERENCE),
  ),
);
mapSet(
  CORE_EVENT_NAMED_LIMITS,
  ...
  limitRecord(
    limitField("intentId", LIMIT_IDENTIFIER),
    limitField("noEffect", LIMIT_ADAPTER_EVIDENCE),
    limitField("attesterId", LIMIT_IDENTIFIER),
    limitField("evidenceReference", LIMIT_EVIDENCE_REFERENCE),
  ),
);
mapSet(
  ...
  limitRecord(
    limitField("intentId", LIMIT_IDENTIFIER),
    limitField("issuerAgentId", LIMIT_IDENTIFIER),
    limitField("runtimeSessionId", LIMIT_IDENTIFIER),
    limitField("roleId", LIMIT_IDENTIFIER),
    limitField("roleTenureId", LIMIT_IDENTIFIER),
  ),
...
mapSet(
  ...
  "ATTEMPT_DUTY_CREATED",
  limitRecord(
    limitField("record", LIMIT_ATTEMPT_DUTY_RECORD),
    limitField("actorId", LIMIT_IDENTIFIER),
    limitField("administrativeAuthorization", LIMIT_ADMINISTRATIVE_AUTHORIZATION),
  ),
);
  ...
  switch (node.kind) {
    ...
    case "RECORD": {
      ...
      for (let index = 0; index < node.fields.length; index += 1) {
        const field: NamedLimitField = node.fields[index]!;
        const name = field[0];
        if (
    ...
    case "LIST": {
      ...
      if (node.maximum !== undefined && value.length > node.maximum) {
        return v.failLimit(
          path,
          `exceeds the ${node.maximum}-member ${node.boundName ?? "named list"} bound`,
        );
      }
      if (node.member === undefined) return true;
      for (let index = 0; index < value.length; index += 1) {
    ...
    case "TAGGED_RECORD": {
      ...
      if (!Object.hasOwn(value, node.tagField)) return true;
      const tag = value[node.tagField];
      if (typeof tag !== "string") return true;
      for (let index = 0; index < node.variants.length; index += 1) {
        const variant: NamedLimitVariant = node.variants[index]!;
        if (variant[0] === tag) {
 ...
 */
const preflightEventNamedLimits = (
  v: ShapeValidator,
  type: CoreEventType,
  value: unknown,
  path = "$.data",
...
const LIMIT_RECEIPT_PAYLOAD = limitRecord(
  limitField("domain", LIMIT_DOMAIN),
  limitField("issuer", limitRecord(
    limitField("keyId", LIMIT_IDENTIFIER),
    limitField("agentId", LIMIT_IDENTIFIER),
    limitField("runtimeSessionId", LIMIT_IDENTIFIER),
    limitField("roleId", LIMIT_IDENTIFIER),
  ...
  limitField("intent", LIMIT_TRANSACTION_INTENT),
  limitField("authorization", limitRecord(
    limitField("policyVersion", LIMIT_IDENTIFIER),
    limitField("proof", LIMIT_AUTHORIZATION_PROOF),
  )),
  limitField("result", limitRecord(
    limitField("consumptionEventId", LIMIT_IDENTIFIER),
    limitField("adapterId", LIMIT_IDENTIFIER),
    limitField("transactionReference", LIMIT_IDENTIFIER),
    limitField("acknowledgment", LIMIT_ADAPTER_EVIDENCE),
  )),
...
const receiptPayload = (v: ShapeValidator, value: unknown, path: string): boolean => {
  ...
  return issuer !== undefined && authorization !== undefined &&
    ...
    literal(v, authorization.rootRecognitionPolicy, `${path}.authorization.rootRecognitionPolicy`,
      ["declared-principal-root/0.2"]) &&
 ...
 */
export const validateCapturedPortableReceiptValue = (
  capture: { readonly capturedRecordKeys: (value: unknown) => readonly string[] | undefined },
  kind: "ARTIFACT" | "DOMAIN" | "HISTORY_HEAD" | "IDENTIFIER" | "TIME",
  value: unknown,
): "VALID" | "INVALID" | "LIMIT" | "UNSUPPORTED" => {
  const validator = new ShapeValidator(capture.capturedRecordKeys);
  const result = (valid: boolean): "VALID" | "INVALID" | "LIMIT" =>
    valid ? "VALID" : validator.failureKind === "LIMIT" ? "LIMIT" : "INVALID";
  switch (kind) {
    case "DOMAIN":
    ...
    case "ARTIFACT": {
      ...
      if (artifact === undefined) return result(false);
      if (!contentHash(validator, artifact.contentHash, "$.contentHash") ||
        !signature65(validator, artifact.signature, "$.signature")) return result(false);
      ...
      if (!identifier(validator, payload.schemaVersion, "$.payload.schemaVersion") ||
        !identifier(validator, payload.signatureScheme, "$.payload.signatureScheme")) return result(false);
      ...
      if (payload.schemaVersion !== "continuity-receipt/0.2" ||
        payload.signatureScheme !== "eip191-personal-sign-keccak256") return "UNSUPPORTED";
...
export const validateCapturedPortableQueryInput = (
  capture: { readonly capturedRecordKeys: (value: unknown) => readonly string[] | undefined },
  ...
  for (const name of ["evaluationEvents", "observedEvents"]) {
    ...
    for (let position = 0; position < history.length; position += 1) {
      ...
      if (!preflightNamedLimitNode(v, event.id, `$.${name}[${position}].id`, LIMIT_IDENTIFIER)) return false;
      if (position === 0 && event.type === "DEPLOYMENT_INITIALIZED" &&
          !preflightEventNamedLimits(v, "DEPLOYMENT_INITIALIZED", event.data, `$.${name}[0].data`)) return false;
    }
  }
  ...
  if (isPlainRecord(value.disclosure)) {
    for (const name of ["includedFields", "withheldFields"]) {
      ...
      if (Array.isArray(list)) for (let i = 0; i < list.length; i += 1) {
        if (!preflightNamedLimitNode(v, list[i], `$.disclosure.${name}[${i}]`, LIMIT_IDENTIFIER)) return false;
      }
    }
  }
  if (!scalar(value.evaluationTime)) return false;
  if (kind === "SURVIVES") return identifier(v, value.targetAgentId, "$.targetAgentId");
  if (!preflightNamedLimitNode(v, value.authorizationDomain, "$.authorizationDomain", LIMIT_DOMAIN) ||
      !authorizationDomain(v, value.authorizationDomain, "$.authorizationDomain")) return false;
  const request = v.record(value.request, "$.request", ["actorId", "action", "resource", "claimedAt"],
    ["amount", "counterpartyId", "termsCommitment"]);
  if (request === undefined) return false;
  const scalarKeys = ["actorId", "action", "resource", "claimedAt", "amount", "counterpartyId"];
  for (let i = 0; i < scalarKeys.length; i += 1) {
    if (request[scalarKeys[i]!] !== undefined && !scalar(request[scalarKeys[i]!])) return false;
  }
  ...
  for (let i = 0; i < ids.length; i += 1) {
    if (!preflightNamedLimitNode(v, request[ids[i]!], `$.request.${ids[i]!}`, LIMIT_IDENTIFIER)) return false;
  }
  if (value.consequentialBinding === undefined) return true;
  const b = v.record(value.consequentialBinding, "$.consequentialBinding",
    ["runtimeSessionId", "controlEpoch", "roleId", "roleTenureId"],
    ["credentialKeyId", "intentId", "nonce", "runtimeSignature"]);
...
export const validateCapturedPortableQueryDisclosure = (
  capture: { readonly capturedRecordKeys: (value: unknown) => readonly string[] | undefined },
  value: unknown,
): boolean => {
  const v = new ShapeValidator(capture.capturedRecordKeys);
  const record = v.record(value, "$.disclosure", ["mode", "includedFields", "withheldFields"]);
  if (record === undefined || record.mode !== "PUBLIC_MINIMAL") return false;
  const validList = (candidate: unknown): candidate is readonly string[] => {
    if (!Array.isArray(candidate) || candidate.length > MAX_LIST_MEMBERS) return false;
    for (let i = 0; i < candidate.length; i += 1) {
      if (!identifier(v, candidate[i], "$.disclosure.fields") ||
        (i > 0 && compareProtocolStrings(candidate[i - 1] as string, candidate[i] as string) >= 0)) return false;
    }
    return true;
  };
  if (!validList(record.includedFields) || !validList(record.withheldFields)) return false;
  for (let i = 0; i < record.withheldFields.length; i += 1) {
    if (arrayIncludes(record.includedFields, record.withheldFields[i]!)) return false;
  }
  return true;
};
  ...
  switch (type) {
    ...
    case "SUCCESSION_RULE_DECLARED": {
      ...
      return record !== undefined &&
        ...
        enumList(
          ...
          `${path}.permittedEventTypes`,
          ["AGENT_TERMINATED", "ROLE_TRANSFERRED", "OBLIGATION_PERFORMANCE_ASSIGNED"],
          true,
        );
    }
    case "AGENT_APPOINTED": {
      const record = v.record(
        value,
        path,
        ["agentId", "roleId", "roleTenureId", "tenureNumber", "principalId"],
        ["successionRuleId"],
      );
      return record !== undefined &&
        identifier(v, record.agentId, `${path}.agentId`) &&
    ...
    case "TRANSACTION_OUTCOME_RECORDED": {
      ...
      if (record !== undefined && record.status !== "FAILED" &&
          v.record(value, path, ["intentId", "status", "attesterId", "evidenceReference"]) === undefined) return false;
      return record !== undefined &&
        identifier(v, record.intentId, `${path}.intentId`) &&
...
type EnvelopeFieldCapture =
  | Readonly<{ readonly ok: true; readonly value: unknown }>
  | Readonly<{ readonly ok: false; readonly reason: string }>;

const captureEnvelopeField = (
  source: object,
  key: (typeof EVENT_ENVELOPE_FIELDS)[number],
): EnvelopeFieldCapture => {
  ...
  if (descriptor.get === undefined) return { ok: true, value: undefined };
  try {
    return {
      ok: true,
...
const invalidEvent = (
  code: EventShapeFailureCode,
  reason: string,
  failureKind?: "SEMANTIC" | "LIMIT",
): EventShapeValidationResult => {
  const result: EventShapeValidationResult = { ok: false, code, reason };
  return failureKind === "LIMIT" ? brandSchemaLimitFailure(result) : result;
};

const invalidGenesisProbe = (
  reason: string,
  failureKind?: "SEMANTIC" | "LIMIT",
): GenesisVersionProbeValidationResult => {
  const result: GenesisVersionProbeValidationResult = {
    ok: false,
 ...
 */
export const validateCapturedReplayBaseCandidate = (
  candidate: unknown,
  eventPosition: number,
  source: CapturedBaseEventSource,
): BaseEventShapeValidationResult => {
  ...
  if (capturedEnvelopeKeys !== undefined) {
    const expectedEnvelopeKeys = createSet<string>();
  ...
  if (
    !identifier(validator, record.id, `${path}.id`) ||
  ...
  const baseEvent = Object.freeze({
    eventPosition,
    event: record as unknown as AcceptedCanonicalBaseEventShape,
  });
  weakMapSet(CAPTURED_BASE_EVENT_SOURCES, baseEvent, Object.freeze(source));
  return { ok: true, baseEvent };
};

export const validateCapturedReplayBaseEvent = (
  capture: CapturedCanonicalReplayBody,
  eventPosition: number,
): BaseEventShapeValidationResult => {
  if (
    !isCapturedCanonicalReplayBody(capture) ||
  ...
  return validateCapturedReplayBaseCandidate(
    capture.value.events[eventPosition],
    eventPosition,
    {
...
export const validateIncrementallyCapturedReplayBaseEvent = (
 ...
 */
export const validateCapturedGenesisVersionProbe = (
  ...
  if (!preflightEventNamedLimits(
    ...
    return invalidGenesisProbe(
      validator.problem ?? `${path}: invalid GenesisVersionProbe`,
      validator.failureKind,
    );
  }
  ...
  if (
    ...
    (record.rootRecognitionPolicy !== "declared-principal-root/0.2" &&
      !validator.fail(
        `${path}.rootRecognitionPolicy`,
        "must equal declared-principal-root/0.2",
      )) ||
  ...
  const probe = baseEvent.event as AcceptedGenesisVersionProbe;
  return {
    ok: true,
    probe,
 ...
 */
export const preflightCapturedPortableReceiptHistoryLimits = (
  capture: CapturedBaseEventSource,
    ...
    if (history.length > 0) {
      ...
      if (isPlainRecord(first) && first.type === "DEPLOYMENT_INITIALIZED") {
        ...
        if (base.ok) {
          ...
          if (isCoreSchemaLimitFailure(discovery)) discoveryLimit = true;
          if (discovery.ok) {
            exactVersions = versionSet(
              new ShapeValidator(capture.capturedRecordKeys), discovery.versions,
              `$.histories[${index}][0].data.versions`,
            );
          }
        }
      }
  ...
  if (discoveryLimit) return "LIMIT";
  for (let index = 0; index < histories.length; index += 1) {
    ...
    for (let position = 0; position < history.length; position += 1) {
      ...
      if (isPlainRecord(event) && isCoreEventType(event.type) &&
          !preflightEventNamedLimits(validator, event.type, event.data,
            `$.histories[${index}][${position}].data`) && validator.failureKind === "LIMIT") {
        return "LIMIT";
      }
    }
  }
 ...
 */
export const refineCapturedCoreEvent = (
  baseEvent: CapturedCanonicalBaseEvent,
): EventShapeValidationResult => {
  const source = weakMapGet(CAPTURED_BASE_EVENT_SOURCES, baseEvent);
  if (source === undefined) {
    return invalidEvent(
      "EVENT_ENVELOPE_INVALID",
      "$: event is not a captured replay base event",
    );
  }
  if (!isCoreEventType(baseEvent.event.type)) {
    return invalidEvent(
      "EVENT_TYPE_UNSUPPORTED",
      `$.type: unsupported 0.2 EventType ${jsonStringify(baseEvent.event.type)}`,
    );
  }
  const validator = new ShapeValidator(source.capturedRecordKeys);
  if (
    !preflightEventNamedLimits(
      validator,
      baseEvent.event.type,
      baseEvent.event.data,
    ) ||
    ...
    return invalidEvent(
      "EVENT_DATA_INVALID",
      validator.problem ?? "$.data: invalid closed event payload",
      validator.failureKind,
    );
  }
  const event = baseEvent.event as AcceptedCanonicalEventShape;
  return {
    ok: true,
 ...
 */
export const validateCanonicalEventShape = (input: unknown): EventShapeValidationResult => {
  if (input === null || typeof input !== "object") {
    return invalidEvent(
      "EVENT_ENVELOPE_INVALID",
      "$: event must be a plain protocol record",
    );
  }
  ...
  try {
    ...
    return invalidEvent(
      "EVENT_ENVELOPE_INVALID",
      "$: failed event prototype capture",
    );
  }
  ...
  try {
    ...
    return invalidEvent(
      "EVENT_ENVELOPE_INVALID",
      "$: failed event own-key capture",
    );
  }
  const expectedFields = createSet<string>();
  for (let index = 0; index < EVENT_ENVELOPE_FIELDS.length; index += 1) {
  ...
  for (let index = 0; index < ownKeys.length; index += 1) {
    ...
    if (typeof key !== "string") {
      return invalidEvent(
        "EVENT_ENVELOPE_INVALID",
        "$: event has a non-string field",
      );
    }
    if (!setHas(expectedFields, key)) {
      return invalidEvent(
        "EVENT_ENVELOPE_INVALID",
        "$: event has an undeclared field",
      );
    }
  }
  const observedFields = createSet<string>();
  for (let index = 0; index < ownKeys.length; index += 1) {
    setAdd(observedFields, ownKeys[index] as string);
  }
  for (let index = 0; index < EVENT_ENVELOPE_FIELDS.length; index += 1) {
  ...
  if (rawData === null || typeof rawData !== "object") {
    return invalidEvent(
      "EVENT_ENVELOPE_INVALID",
      "$.data: must be a plain protocol record",
    );
  }
  let dataCapture: ReturnType<typeof captureBoundedCanonicalEventData>;
  try {
    dataCapture = captureBoundedCanonicalEventData(
      rawData,
      input,
    ...
    if (isCanonicalCaptureLimitError(error, "$", MAX_EVENT_DATA_BYTES)) {
      return invalidEvent(
        "EVENT_DATA_INVALID",
        `$.data: exceeds the ${MAX_EVENT_DATA_BYTES}-byte event-data byte limit`,
      );
    }
    return invalidEvent(
      "EVENT_DATA_INVALID",
      "$.data: failed canonical base capture",
    );
  }
  ...
  if (!isCoreEventType(typeCapture.value)) {
    return invalidEvent(
      "EVENT_TYPE_UNSUPPORTED",
      `$.type: unsupported 0.2 EventType ${jsonStringify(typeCapture.value)}`,
    );
  }

  const rootRecordKeys = Object.freeze(["data", "id", "timestamp", "type"]);
  const frozenEvent = Object.create(null) as Record<string, unknown>;
  ...
  if (
    ...
    return invalidEvent(
      "EVENT_DATA_INVALID",
      dataValidator.problem ?? "$.data: invalid closed event payload",
      dataValidator.failureKind,
    );
  }

  const frozen = frozenEvent as unknown as AcceptedCanonicalEventShape;
